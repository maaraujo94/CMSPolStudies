...
void plotRes()
{
  ...
  // draw A(pT)
  c->SetLogy();
  TH1F *fa = c->DrawFrame(pTBins[0], 0.3, pTBins_vhpt[nBinspT_vhpt], 9);
  ...
  TLine *trans1_A = new TLine(pTBins_hpt[0], 0.3, pTBins_hpt[0], 9);
  trans1_A->SetLineColor(kBlack);
  trans1_A->SetLineStyle(kDashed);
  trans1_A->Draw();
  TLine *trans2_A = new TLine(pTBins_vhpt[0], 0.3, pTBins_vhpt[0], 9);
