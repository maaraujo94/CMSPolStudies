...
double myFunction(double lth, double lph, double ltp, double A, double id)
{
  ...
  int nBinsX = h_J[0]->GetNbinsX(), nBinsY = h_J[0]->GetNbinsY();
  double minX = h_J[0]->GetXaxis()->GetBinLowEdge(1);
  double maxX = h_J[0]->GetXaxis()->GetBinUpEdge(nBinsX);
  double dX = (maxX-minX)/(nBinsX);
  double minY = h_J[0]->GetYaxis()->GetBinLowEdge(1);
  double maxY = h_J[0]->GetYaxis()->GetBinUpEdge(nBinsY);
  ...
  for(int iX = 0; iX < nBinsX; iX++) {
    ...
    for(int iY = 0; iY < nBinsY; iY++) {
      ...
      hval = h_J[i]->GetBinContent(iX+1, iY+1);
      if(hval==0) continue;

      if(iX+2 < nBinsX)
	if(h_J[i]->GetBinContent(iX+2, iY+1) == 0)
	  continue;
      if(iY+2 < nBinsX)
	if(h_J[i]->GetBinContent(iX+1, iY+2) == 0)
	  continue;
      if(iX > 0)
	if(h_J[i]->GetBinContent(iX, iY+1) == 0)
	  continue;
      if(iY > 0)
	if(h_J[i]->GetBinContent(iX+1, iY) == 0)
	  continue;
      
      wval = f_w[i]->Integral(xmin, xmax, ymin, ymax); 
      herr = h_J[i]->GetBinError(iX+1, iY+1);
...
void doFit()
{
  ...
  for(int i = 0; i < nPtBins; i++) {
    infile->GetObject(Form("h_J_%d", i), h_J[i]);
    h_J[i]->SetDirectory(0);
  }
  infile->Close();
  ...
  for(int i = 0; i < nPtBins; i++) {
    //for(int i = 0; i < 1; i++) {
    double in_a = h_J[i]->GetBinContent(nBinsX, nBinsY);
    
    fit->SetParameter(0, "l_th", 0.1, 0.01, -1, 1);
    fit->SetParameter(1, "l_ph", -0.1, 0.01, -1, 1);
    fit->SetParameter(2, "l_tp", 0.01, 0.01, -1, 1);
	...
	ymin = minY+dY*iY;
	ymax = minY+dY*(iY+1);
	if(h_J[i]->GetBinContent(iX+1, iY+1)==0) continue;
	if(iX+2 < nBinsX)
	  if(h_J[i]->GetBinContent(iX+2, iY+1) == 0)
	    continue;
	if(iY+2 < nBinsX)
	...
	double hval = h_J[i]->GetBinContent(iX+1, iY+1);
	double  herr = h_J[i]->GetBinError(iX+1, iY+1);
