...
void indFit()
{
  ...
  ifstream in;
  string dataS;
  in.open("../cosMax/cos_max.txt");
  double minC[nBinsY], aux;
  ...
  double pt[nBinsY], ept[nBinsY], zero[nBinsY];
  ...
  for(int i = 0; i < nBinsY; i++) {
    ...
    TLine *c_lim = new TLine(minC[i], 0, minC[i], pHist[0][i]->GetMaximum());
  ...
  for(int i = 0; i < nBinsY; i++) {
    ...
    for(int i_cos = 0; i_cos < nBinsX; i_cos++) {
      if(xv[i_cos] < minC[i]) gNP_pull->RemovePoint(1);
    }
    ...
    for(int i_cos = 0; i_cos < nBinsX; i_cos++) {
      if(xv[i_cos] < minC[i]) gNP_dev->RemovePoint(1);
    }
    ...
    for(int i_cos = 0; i_cos < nBinsX; i_cos++) {
      if(xv[i_cos] < minC[i]) gPR_pull->RemovePoint(1);
    }
    ...
    for(int i_cos = 0; i_cos < nBinsX; i_cos++) {
      if(xv[i_cos] < minC[i]) gPR_dev->RemovePoint(1);
    }
    ...
    TLine *c_limNP = new TLine(minC[i], 0, minC[i], pHist[1][i]->GetMaximum());
    ...
    TLine *c_limPR = new TLine(minC[i], 0, minC[i], pHist[3][i]->GetMaximum());
  ...
  TGraphErrors *graphCm = new TGraphErrors(nBinsY, pt, minC, ept, zero);
  graphCm->SetName(Form("graph_cMin"));
  graphCm->Write();
