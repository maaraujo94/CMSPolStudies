...
void fitBkgCosth()
{
  ...
  for(int i_inp = 0; i_inp < 2; i_inp++) {
	...
	pHist[i]->SetTitle(Form("Full %s/MC bin %d: [%.0f, %.0f] GeV", lbl[i_inp].c_str(), i+1, yBins[i], yBins[i+1]));
	...
	c->SaveAs(Form("plots/%s/cth_%d_pt%d.pdf", lbl[i_inp].c_str(), i_f+1, i+1));
	c->Clear();
	...
	fp->SetTitle(Form("Full %s pulls (%.0f < p_{T} < %.0f GeV)", lbl[i_inp].c_str(), yBins[i], yBins[i+1]));
	...
	c->SaveAs(Form("plots/%s/pulls_%d_pt%d.pdf", lbl[i_inp].c_str(), i_f+1, i+1));
	c->Clear();
	...
	c->SaveAs(Form("plots/%s/dev_%d_pt%d.pdf", lbl[i_inp].c_str(), i_f+1, i+1));
	c->Clear();
      }
      ...
      ofstream fout;
      fout.open(Form("text_output/cos_%s_%d.tex", lbl[i_inp].c_str(), i_f));
	...
	fout << Form(" & %.0f", chi2[i]) << "/" << Form("%.0f", ndf[i]) << "\\\\\n";
      ...
      graph_N->SetName(Form("fit_%d_N", i_f));
      graph_N->Write();
      graph_l2->SetName(Form("fit_%d_l2", i_f));
      graph_l2->Write();
      graph_l4->SetName(Form("fit_%d_l4", i_f));
      graph_l4->Write();
      graph_C->SetName(Form("fit_%d_chi", i_f));
      graph_C->Write();
      graph_ND->SetName(Form("fit_%d_ndf", i_f));
      graph_ND->Write();
      graph_P->SetName(Form("fit_%d_chiP", i_f));
      graph_P->Write();
      fOut->Close();
