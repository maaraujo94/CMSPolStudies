...
void newMCmass_Gfree()
{
  ...
  for(int ip = 0; ip < nPtBins; ip++)
    h_m1d[ip] = new TH1D(Form("mH%.0f", ptBins[ip]), Form("Full MC M(#mu#mu) (%.1f < p_{T} < %.1f GeV)",  ptBins[ip], ptBins[ip+1]), mbins, lowm, him);
  ...
  TFile *fout = new TFile("files/MCfit_Gfree.root", "recreate");
  h_m2d->Fit("f_cb", "R");
  ...
  for(int i_pt = 0; i_pt < nPtBins; i_pt++) {
    ...
    c->SaveAs(Form("plots/MCMass/fit_Gf/CBG_pt%d.pdf", i_pt));
    c->Clear();
    ...
    c->SaveAs(Form("plots/MCMass/fit_Gf/pullsG_pt%d_dep.pdf", i_pt));
    c->Clear();
