...
void newMCmass_4fix()
{
  ...
  double par_v[] = {1., 0.7, 3.097, 2e-2, 4e-2, n_v, 2};
  ...
  TFile *fout = new TFile("files/MCfit_4fix.root", "recreate");
  h_m2d->Fit("f_cb", "R");
  ...
  for(int i_pt = 0; i_pt < nPtBins; i_pt++) {
    ...
    c->SaveAs(Form("plots/MCMass/fit_4fix/CB_pt%d.pdf", i_pt));
    c->Clear();
    ...
    c->SaveAs(Form("plots/MCMass/fit_4fix/pulls_pt%d_dep.pdf", i_pt));
    c->Clear();
