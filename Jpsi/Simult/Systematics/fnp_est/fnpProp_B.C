// macro to propagate the uncertainties to f_NP analytically - prev model
...
double intfNP(double x, double *par)
{
  ...
  fNP_SR->SetParameters(N_NP, f2, mu, sig1, sig2, tnp_SR);
...
void fnpProp_B()
{
  ...
  TFile *inNP = new TFile("files/ltfitresB.root");
  int n_par = 7; 
  // get fitres to get cov in each pT bin
  TFitResult **fitres = new TFitResult*[nPtBins];
  for(int i = 0; i < nPtBins; i++)
    inNP->GetObject(Form("fitres_%.0f", ptBins[i]), fitres[i]);
  inNP->Close();
  ...
  for(int i_pt = 0; i_pt < nPtBins; i_pt++) {
    ...
    double par_vec[6], epar_vec[6], cov_mat[6][6];
    for(int i_p = 0; i_p < 6; i_p++) {
      par_vec[i_p] = fitres[i_pt]->Parameter(i_p+1);
      epar_vec[i_p] = fitres[i_pt]->ParError(i_p+1);
      for(int j_p = 0; j_p < 6; j_p++) {
	cov_mat[i_p][j_p] = fitres[i_pt]->GetCovarianceMatrix()[i_p+1][j_p+1];
      }
    }
    double *cov_ptr = cov_mat[0];

    fnp[i_pt] = intfNP(0, par_vec)/evt_all[i_pt] * 100.;
    efnp[i_pt] = parErr(6, intfNP, 0, par_vec, epar_vec, cov_ptr)/evt_all[i_pt] * 100.;
  ...
  // get 1-f_bkg^NP
  TH2D *h_fc = new TH2D("h_fc", "h_fc", nBinsX, minX, maxX, nPtBins, ptBins);
  for(int i_x = 0; i_x < nBinsX; i_x++) {
    for(int i_y = 0; i_y < nPtBins; i_y++) {
  ...
  // get corrected f_NP
  TH2D *h_fNPc = (TH2D*)h_fnp2d->Clone("h_fNPc");
  h_fNPc->Multiply(h_fc);
  
  TFile *fout = new TFile("files/NPFrac_B.root", "recreate");
