...
void fbkgProp()
{
  ...
  TFile *inBG = new TFile("files/mfit_bkg.root");
  int n_par = 2;
  ...
  for(int i_pt = 0; i_pt < n_pt; i_pt++) {
    ...
    TFitResult* fitres = (TFitResult*)inBG->Get(Form("fitres_%d", i_pt));
    ...
    fe = sqrt(fe);
    cout << i_pt << " " << fv/evt_all*100 << " +/- " << fe/evt_all*100 << endl;
  ...
  TFile *infile = new TFile("/home/mariana/Documents/2020_PhD_work/CERN/CMSPolStudies/Jpsi/Simult/PR_fit/files/histoStore.root");
  TH2D *hist = new TH2D();
