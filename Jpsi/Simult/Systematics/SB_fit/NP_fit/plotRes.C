...
void plotRes()
{
  ...
  // draw comparison between base and new lth
  TFile *fIndB = new TFile("../../../PR_fit/files/finalFitRes.root");
  TGraphErrors *graph_lthB = (TGraphErrors*)fIndB->Get("graph_lambda_NP");
  fIndB->Close();

  TH1F *flB = c->DrawFrame(pTBins[0]-5, -1, pTBins[nBinspT], 1);
  flB->SetXTitle("p_{T} (GeV)");
  flB->SetYTitle("#lambda_{#theta}");
  flB->GetYaxis()->SetTitleOffset(1.3);
  flB->GetYaxis()->SetLabelOffset(0.01);
  flB->SetTitle("Run 2 #lambda_{#theta} (non-prompt J/#psi)");

  graph_lth[1]->SetLineColor(kBlack);
  graph_lth[1]->SetMarkerColor(kBlack);
  graph_lth[1]->Draw("p same");

  graph_lthB->SetLineColor(kBlue);
  graph_lthB->SetMarkerColor(kBlue);
