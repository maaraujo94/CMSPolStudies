...
void fbkgProp_NP()
{
  ...
  // get baseline frac too
  TFile *finB = new TFile("../../bkgFits/files/bkgFrac_NP.root");
  TH1D* h_fbkg_B = (TH1D*)finB->Get("fbkg_unc");
  h_fbkg_B->SetName("fbkg_unc_B");
  h_fbkg_B->SetDirectory(0);
  finB->Close();
  ...
  h_fbkg->SetMarkerColor(kBlue);
  h_fbkg->SetLineColor(kBlue);
  h_fbkg->Draw("e1 same");

  h_fbkg_B->Scale(100);
  h_fbkg_B->SetStats(0);
  h_fbkg_B->SetMarkerStyle(20);
  h_fbkg_B->SetMarkerSize(.5);
  h_fbkg_B->SetMarkerColor(kBlack);
  h_fbkg_B->SetLineColor(kBlack);
  h_fbkg_B->Draw("e1 same");
