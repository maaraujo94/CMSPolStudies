...
double mmod_func(double *x, double *par)
{
  ...
  double mu2 = par[11*nPtBins]; // added a constant mu2, for indep means
...
void newNPmass()
{
  ...
  double par_v[] =  {1.,   0.55, 3.095, 1e-4,   1e-4,   n_v, alpha_v, 1.,   0.3,      fG_v, 1., 3.095};
  double par2_v[] = {1.,   1.,   1.,    2e-2,   3e-2,   1.,  1.,      1.,   1.,       1.,   1e-1, 1};
  ...
  for(int i = 0; i < nPtBins; i++) {
	...
	if((j < 3 || j == 6 || j == 11)  && i > 0) f_cb->FixParameter(j*nPtBins+i, par_v[j]);
  ...
  for(int i_pt = 0; i_pt < nPtBins; i_pt++) {
	...
	epars[j][i_pt] = f_cb->GetParError(j*nPtBins+i_pt);
      ...
      else if ( j == 1 || j == 2 || j == 5 || j == 6 || j == 9 || j == 11) { // constant parameters mu, f, n, alpha, fG
