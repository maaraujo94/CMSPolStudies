...
void storeRdpT()
{
  ...
  // output histos
  TH2D* r_RdpT = new TH2D("r_RdpT", "reco p_{T} > 50", 75, 0, 0.3, 75, 0, 120);
  TH2D* t_RdpT = new TH2D("t_RdpT", "trig p_{T} > 50", 75, 0, 0.3, 75, 0, 120);
  ...
  r_RdpT->GetXaxis()->SetTitle("#deltaR_{pt}");
  r_RdpT->GetYaxis()->SetTitle("|#deltap_{T}|");
  t_RdpT->GetXaxis()->SetTitle("#deltaR_{pt}");
  t_RdpT->GetYaxis()->SetTitle("|#deltap_{T}|");
  
  TFile *fout = new TFile("RdpTstore.root", "recreate");
  r_RdpT->Write();
  t_RdpT->Write();
  fout->Close();
