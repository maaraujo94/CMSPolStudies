...
void indFit_NP()
{
  ...
  TF1 *cosMin = new TF1("cosMin", "cminf(x, [0], [1], [2], [3])", yBins_f[0]-10, yBins_f[nBinsY]+10);

  // the cycle to fit each bin and store fit results
  TCanvas *c = new TCanvas("", "", 700, 700);    
  TFile *outfile_f = new TFile("files/finalFitRes_NP.root", "recreate");
  ...
  for(int i = 0; i < nBinsY; i++) {
    ...
    c->SaveAs(Form("plots/ratioFinal_NP/bin_%d.pdf", i));
    c->Clear();
    cout << endl << endl;
  }
  ...
  for(int i = 0; i < 3; i++) {
    infile->GetObject(Form("cHistNP%s", lbl[i].c_str()), h_fit[i]);
  ...
  // the cycle to fit each bin and store fit results
  TFile *outfile = new TFile("files/finalFitRes_NP.root", "update");
  ...
  for(int i = 0; i < nBinsY; i++) {
    ...
    c->SaveAs(Form("plots/ratioFinal_NP/bin_c_%d.pdf", i));
    c->Clear();
    cout << endl << endl;
  }
