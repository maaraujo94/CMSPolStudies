...
// on the fine AND coarse bins (using same fit |costheta| region)
...
void fullIndFit()
{
  ...
  // PART 1: the coarse-binned results
  ...
  // PART 2: the fine-binned results
  ...
  // get the binning
  nBinsX = h_fit_f[0]->GetNbinsX();
  nBinsY = h_fit_f[0]->GetNbinsY();
  ...
  for(int i_t = 0; i_t < 3; i_t++) {
    for(int i = 1; i <= nBinsY; i++) {
      ...
      pHist_f[i_t][i-1]->SetTitle(Form("%s bin %d: [%.0f, %.0f] GeV", t_name[i_t].c_str(), i, yBins_f[i-1], yBins_f[i]));
    }
  }
   
  // the cycle to fit each bin and store fit results
  TFile *outfile_f = new TFile("files/finalFitRes_f.root", "recreate");
  ...
  for(int i = 0; i < nBinsY; i++) {
    ...
    // plotting everything
    pHist_f[0][i]->SetTitle(Form("|cos#theta| (%.0f < p_{T} < %.0f GeV)", pMin, pMax));
    ...
    c->SaveAs(Form("plots/ratioFinal/bin_f_%d.pdf", i));
    c->Clear();
    cout << endl << endl;
  }
