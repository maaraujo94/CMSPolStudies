#import "/home/mariana/Documents/2020_PhD_work/CERN/CMSPolStudies/Jpsi/2018/cosMax/imp_jumpF.C"
...
void indFit()
{
  ...
  TH2D **h_fit = new TH2D*[2];
  TFile *infile = new TFile("files/mcComp.root");
  infile->GetObject(Form("rH"), h_fit[0]);
  h_fit[0]->SetDirectory(0);
  infile->Close();
  TFile *infile2 = new TFile("/home/mariana/Documents/2020_PhD_work/CERN/CMSPolStudies/Jpsi/2018/PR_fit/files/histoStore.root");
  infile2->GetObject(Form("rNPH"), h_fit[1]);
  //TFile *infile2 = new TFile("/home/mariana/Documents/2020_PhD_work/CERN/CMSPolStudies/Jpsi/2018/PR_fit/files/bkgSubRes.root");
  //infile2->GetObject(Form("h_NP"), h_fit[1]);
 h_fit[1]->SetDirectory(0);
  ...
  ifstream in;
  string dataS;
  in.open("/home/mariana/Documents/2020_PhD_work/CERN/CMSPolStudies/Jpsi/2018/cosMax/cosMaxFitRes.txt");
  getline(in, dataS);
  getline(in, dataS);
  double maxPar[3], aux;
  ...
  for(int i = 0; i < nBinsY; i++) {
    ...
    // fit the 4 functions
    pHist[1][i]->Scale(pHist[0][i]->GetBinContent(1)/pHist[1][i]->GetBinContent(1));
    for(int i_t = 0; i_t < 2; i_t++) {
    ...
    TLatex lc;
    lc.SetTextSize(0.03);
    lc.DrawLatex(0.1, pHist[0][i]->GetMaximum()*0.9, Form("#lambda_{#theta}^{NP new} = %.3f #pm %.3f", parL[0][i], eparL[0][i]));
    lc.DrawLatex(0.1, pHist[0][i]->GetMaximum()*0.8, Form("#lambda_{#theta}^{NP old} = %.3f #pm %.3f", parL[1][i], eparL[1][i]));
    ...
    leg->AddEntry(pHist[0][i], "NP new", "pl");
    leg->AddEntry(pHist[1][i], "NP old", "pl");
    leg->Draw();

    for(int i_t = 0; i_t < 2; i_t++)
      pHist[i_t][i]->Write();
 
    c->SaveAs(Form("plots/bin_%d.pdf", i));
    c->Clear();
    cout << endl << endl;
  }
