...
void calcSys_NP()
{
  ...
  graph_lth[1] = (TGraphErrors*)fIndf1->Get(Form("graph_lambda_NP"));
  fIndf1->Close();
  TFile *fIndf2 = new TFile("../../../Simult_eff2/PR_fit/files/finalFitRes.root");
  graph_lth[2] = (TGraphErrors*)fIndf2->Get(Form("graph_lambda_NP"));
  fIndf2->Close();
  ...
  for(int i = 0; i < nBinspT; i++) {
    ...
    cout << i << " " << pow(hs_sigEff->GetBinContent(i+1), 2) << endl;
    // 2017-2018 (symmetric)
  ...
  THStack *hsigP = new THStack("hsigP", "Stacked #sigma^{2} (non-prompt)");
  ...
  ofstream ftex;
  ftex.open(Form("sys_unc_NP.tex"));
  ftex << "\\begin{tabular}{c||c|c|c||c|c}\n";
  ftex << "$\\pt$ (GeV) & $\\sigma^{\\text{comb}}$ & $\\sigma^{\\text{eff}}$ & $\\sigma^\\rho$ & $\\sigma_{\\text{sys}}^{\\text{NP}}$ & $\\sigma_{\\text{stat}}^{\\text{NP}}$ \\\\\n";
  ftex << "\\hline\n";
  ...
  fl1->SetTitle("Run 2 #lambda_{#theta} (NP J/#psi)");
  ...
  c->SaveAs("plots/lthNP_full_unc.pdf");
  c->Clear();
  ...
  fl2->SetTitle("Run 2 #lambda_{#theta} (NP J/#psi)");
  ...
  c->SaveAs("plots/lthNP_sep_unc.pdf");
  c->Destructor();

  TFile *outfile = new TFile("lthUnc_NP.root", "recreate");
