...
void plot_fBGNP()
{
  ...
  fIndB->Close();
  // 2 - get lambda values for lower fBGNP_NP
  TFile *fIndL = new TFile("../fBGNP_0p8/files/finalFitRes.root");
  graph_lth[1] = (TGraphErrors*)fIndL->Get(Form("graph_lambda_J"));
  graph_lth_NP[1] = (TGraphErrors*)fIndL->Get(Form("graph_lambda_NP"));
  fIndL->Close();
  // 3 - get lambda values for higher fBGNP_NP
  TFile *fIndH = new TFile("../fBGNP_1p2/files/finalFitRes.root");
  ...
  fl1->SetTitle("prompt #Delta#lambda_{#theta} (f_{bkg}^{NP})");
  ...
  leg->AddEntry(g_lthL, "0.8*f_{bkg}^{NP}", "pl");
  leg->AddEntry(g_lthH, "1.2*f_{bkg}^{NP}", "pl");
  leg->Draw();

  c->SaveAs("lth_absDiff_fBGNP.pdf");
  c->Clear();
  ...
  fl2->SetTitle("non-prompt #Delta#lambda_{#theta} (f_{bkg}^{NP})");
  ...
  c->SaveAs("lthNP_absDiff_fBGNP.pdf");
  c->Clear();
  c->Destructor();
