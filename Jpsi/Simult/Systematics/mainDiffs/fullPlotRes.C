...
void fullPlotRes()
{
  ...
  TFile *fIndR = new TFile("../deltaR/files/finalFitRes_f.root");
  ...
  // make a tgraph out of the difference
  double yearAvg[nBinspT];
  for(int i = 0; i < nBinspT; i++)
    yearAvg[i] = 0.5*(abs(vy_int[0])+abs(vy_int[1]));
  TGraphErrors *yearC = new TGraphErrors(nBinspT, graph_lth[0]->GetX(), za, graph_lth[0]->GetEX(), yearAvg);
  ...
  yearC->SetLineColor(kBlack);
  yearC->SetFillColorAlpha(kBlack, 0.3);
  yearC->Draw("ce3");
