...
void bkgSub_NP()
{
  ...
  TFile *inBkg = new TFile("../../NP_fit/files/bkgCosModel.root");
  ...
  TFile *inFracSB = new TFile("files/bkgFrac_NP.root");
  inFracSB->GetObject("h_fbkg", h_fb2d);
  h_fb2d->SetDirectory(0);
  inFracSB->Close();

  // prepare output
  TFile *fout = new TFile("files/bkgSubRes_NP.root", "recreate");
