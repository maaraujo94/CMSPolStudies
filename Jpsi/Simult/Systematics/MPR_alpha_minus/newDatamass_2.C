...
void newDatamass_2()
{
  ...
  // also get sigmaG since it's being so finicky
  double sigG_v = ((TFitResult*)finF->Get("fitres"))->Parameter(10*nPtBins+1);
  finF->Close();
  alpha_v-=alpha_e;
  ...
  double par_v[] =  {1.,   0.5, 3.095, 1e-4,   1e-4,   n_v, alpha_v, 1.,   0.5,      fG_v, 1.};
  ...
  for(int i = 0; i < nPtBins; i++) {
	...
	// fixing sigG
	if(j == 10 && i== 1) f_cb->FixParameter(j*nPtBins+i, sigG_v);
      }
    }
  }
