...
void fullIndFit()
{
  ...
  TFile *infile = new TFile("files/bkgSubRes_full.root");
  ...
  TFile *outfile = new TFile("files/finalFitRes_full.root", "recreate");
  ...
  for(int i = 0; i < nBinsY; i++) {
    ...
    c->SaveAs(Form("plots/ratioFinal_full/bin_%d.pdf", i));
    c->Clear();
    cout << endl << endl;
  }
