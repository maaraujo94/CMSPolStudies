...
void plotLtPars()
{
  ...
  for(int i = 0; i < n_p; i++) {
    ...
    fp->SetTitle(Form("Full %s", partit[i].c_str()));
  ...
  fchi->SetTitle("Full #chi^{2}/ndf vs p_{T}");
  ...
  fchiP->SetTitle("Full P(#chi^{2}) vs p_{T}");
  ...
  ffr->SetTitle("Full f_{NP} vs p_{T}");
  ...
  fsig->SetTitle("Full #sigma_{m} vs p_{T}");
  ...
  frms->SetTitle("Full rms (Lxy) vs p_{T}");
  ...
  frat->SetTitle("Full #sigma_{2}/#sigma_{1} vs p_{T}");
