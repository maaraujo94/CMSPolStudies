...
void plotCosPars_all()
{
  ...
  for(int i_f = 0; i_f < n_fit; i_f++) { // cycle through fit format
    ...
    for(int i_inp = 0; i_inp < n_inp; i_inp++) { // cycle through SB
      // open file
      TFile *fin = new TFile(Form("files/%s%s_fitres.root", lbl[i_inp].c_str(), filename[i_f].c_str()));	

      // get fit pars
      g_par[i_f][i_inp] = new TGraphErrors*[n_p];
      for(int i_p = 0; i_p < n_p; i_p++) {
	if(i_f == 0) 
	  fin->GetObject(Form("fit_0_%s", parlab[i_p].c_str()), g_par[i_f][i_inp][i_p]);
	else
      ...
      // get fit chi^2
      if(i_f == 0)
	fin->GetObject("fit_2_chiP", g_chi);
      else
