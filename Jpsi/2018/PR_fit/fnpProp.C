...
void fnpProp()
{
  ...
  TH1D *h_fnp_psi = new TH1D("h_fnp_psi", "2018 f_{NP}", nPtBins, ptBins);
  ...
  double b_pars[8];
  TH1D *h_fnp_bkg = new TH1D("h_fnp_bkg", "2018 f_{NP_{bkg}}", nPtBins, ptBins);
  ...
  TH2D *h_fnp2d = new TH2D("h_fnp2d", "2018 f_{NP}", nBinsX, minX, maxX, nPtBins, ptBins);
  for(int i_pt = 0; i_pt < nPtBins; i_pt++) {
