...
void plotRes()
{
  ...
  for(int i = 0; i < ntot; i++) {
    ...
    pHist[i]->SetTitle(Form("NP/MC %s", pHist[i]->GetTitle()));
    pHist[i]->Draw();
  ...
  // draw A(pT)
  c->SetLogy();
  TH1F *fa = c->DrawFrame(pTBins[0], 0.08, pTBins_vhpt[nBinspT_vhpt], 9);
  ...
  TLine *trans1_A = new TLine(pTBins_hpt[0], 0.08, pTBins_hpt[0], 9);
  trans1_A->SetLineColor(kBlack);
  trans1_A->SetLineStyle(kDashed);
  trans1_A->Draw();
  TLine *trans2_A = new TLine(pTBins_vhpt[0], 0.08, pTBins_vhpt[0], 9);
  ...
  // draw chiProb(pT)
  c->SetLogy();
  TH1F *fc = c->DrawFrame(pTBins[0], 0.01, pTBins_vhpt[nBinspT_vhpt], 1);
  ...
  TLine *trans1_C = new TLine(pTBins_hpt[0], 0.01, pTBins_hpt[0], 1);
  trans1_C->SetLineColor(kBlack);
  trans1_C->SetLineStyle(kDashed);
  trans1_C->Draw();
  TLine *trans2_C = new TLine(pTBins_vhpt[0], 0.01, pTBins_vhpt[0], 1);
