...
void mBkg()
{
  ...
  // change n, alpha to MC fit result
  n_v = 1.945;
  alpha_v = 1.830;
  ...
  for(int i_pt = 0; i_pt < nPtBins; i_pt++) {
    ...
    c->SaveAs(Form("plots/NPmass/fit_pt%d.pdf", i_pt));
    c->Clear();
    ...
    c->SaveAs(Form("plots/NPmass/pulls_pt%d.pdf", i_pt));
    c->Clear();
    ...
    c->SaveAs(Form("plots/NPmass/devs_pt%d.pdf", i_pt));
    c->Clear();
  }
