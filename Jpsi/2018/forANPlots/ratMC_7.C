...
void ratMC_7()
{
  ...
  TH1D **MCRat = new TH1D*[nBinsY-1];
  for(int i = 0; i < nBinsY-1; i++) {
    MCRat[i] = (TH1D*)MCHist[i]->Clone(Form("MCR_%d", i));
    MCRat[i]->Sumw2();
    double scF = MCHist[nBinsY-1]->Integral(1, nBinsX/2)/MCHist[i]->Integral(1, nBinsX/2);
    MCRat[i]->Scale(scF);
    MCRat[i]->Divide(MCHist[nBinsY-1]);
  }    
  ...
  c->SaveAs("plots/MC_rat_7.pdf");
  c->Clear();

  // plot the histos
  for(int i = 0; i < nBinsY; i+=2) {
    // scale histo to match full
    MCHist[i]->Scale(MCHist[nBinsY-1]->Integral(1, nBinsX/2)/MCHist[i]->Integral(1, nBinsX/2));
    MCHist[i]->SetStats(0);
  ...
  leg->AddEntry(MCHist[nBinsY-1], Form("Bin %d: [%.0f, %.0f]", nBinsY-1+1, yBins[nBinsY-1], yBins[nBinsY-1+1]), "pl");
  leg->Draw();

  c->SaveAs("plots/MC_hist_7.pdf");
  c->Clear();
