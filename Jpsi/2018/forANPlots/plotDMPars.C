...
void plotDMPars()
{
  ...
  double parmin[] = {0,    1e1, 3090, 0,   32, 1.0, 2.0,  1e4, 0, 0.,  0,   0};
  double parmax[] = {100., 1e4, 3100, 100, 46, 1.4, 2.25, 2e6, 1, 15., 100, 100};
  ...
  for(int i_p = 0; i_p < 10; i_p++) {
    ...
    c->SaveAs(Form("plots/NPmass/par_%s.pdf", parlab[i_p].c_str()));
    c->Clear();
  }
