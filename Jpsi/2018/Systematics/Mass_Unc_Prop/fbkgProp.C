...
void fbkgProp()
{
  ...
  TH1D *h_fbkg = new TH1D("h_fbkg", "2018 f_{bkg}", n_pt, ptBins);
  double ln = 10000;
  const int n_p = 2;
  ...
  fr1->SetTitle("2018 f_{bkg} vs p_{T}");
  ...
  TH2D *h_fbkg2d = new TH2D("h_fbkg2d", "2018 f_{bkg}", nBinsX, minX, maxX, n_pt, ptBins);
  double dpar_v2[n_p2];
  for(int i_pt = 0; i_pt < n_pt; i_pt++) {
    double pt = 0.5*(ptBins[i_pt+1]+ptBins[i_pt]);
    double xpar[] = {pt};
    ...
    cout << fv << " +- " << fe << " fit and " << h_fbkg->GetBinContent(i_pt+1) << " +- " << h_fbkg->GetBinError(i_pt+1) << endl;
  }
  ...
  h_fbkgpt->SetTitle("2018 f_{bkg} vs p_{T}");
  h_fbkgpt->SetStats(0);
