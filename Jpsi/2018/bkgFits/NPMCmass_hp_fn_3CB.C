...
void NPMCmass_hp_fn_3CB()
{
  ...
  double par_v[] = {1., 0.4, 3.097, 2e-2, 3e-2, 2., 1.8, 1e-2, 8e-2, 0.1, 5e-2};
  ...
  f_cb->FixParameter(5, 2.044);
  f_cb->FixParameter(7, 1.0e-01);
  f_cb->FixParameter(8, 4.3e-2);
  ...
  c->SaveAs("plots/NPMCmass/NP3_fit_hpt_fn.pdf");
  c->Clear();
  ...
  c->SaveAs("plots/NPMCmass/NP3_pulls_hpt_fn.pdf");
  c->Clear();
  ...
  f_cb->FixParameter(5, 1.922);
  f_cb->FixParameter(7, 7.0e-2);
  f_cb->FixParameter(8, 4.5e-2);
  h_mNP_hp->Fit("f_cb", "R");
  ...
  c->SaveAs("plots/NPMCmass/NPR3_fit_hpt_fn.pdf");
  c->Clear();
  ...
  c->SaveAs("plots/NPMCmass/NPR3_pulls_hpt_fn.pdf");
  c->Clear();
