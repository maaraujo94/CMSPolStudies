// macro to plot and fit the 2018 J/psi lifetime distribution - use together with ltBkg (imports in ltPerPt)

// MAIN
void ltPerPt_free(double binLow, double binHigh)
{
  ...
  c->SaveAs(Form("plots/lifetime_free/dist_fit_%.0f.pdf", binLow));
  c->Clear();
  ...
  c->SaveAs(Form("plots/lifetime_free/pulls_%.0f.pdf", binLow));
  c->Clear();
  ...
  c->SaveAs(Form("plots/lifetime_free/devs_%.0f.pdf", binLow));
  c->Clear();
  
  ofstream ftable;
  ftable.open("text_output/lt_fit_free.txt", std::ios::app);
