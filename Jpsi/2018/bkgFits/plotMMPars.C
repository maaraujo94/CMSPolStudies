...
void plotMMPars()
{
  ...
  string mode[] = {"free", "dep0", "dep1", "dep2"};
  ...
  double parmin[] = {0, 4.7e-3, 3.096, 0.0, 0.03,  0.6, 1.8};
  double parmax[] = {1, 5e-3,   3.098, 0.05,  0.055, 1.8, 2.5};
  ...
  TGraphErrors **g_sig = new TGraphErrors*[2];
  ...
  for(int i_p = 0; i_p < n_p; i_p++) {
    ...
    if( i_p == 3) {
      ...
      fl->SetYTitle("#sigma (GeV)");
      fl->GetYaxis()->SetTitleOffset(1.3);
      fl->GetYaxis()->SetLabelOffset(0.01);
      fl->SetTitle(Form("2018 #sigma"));
      ...
      g_par[i_p+1][1]->SetLineColor(kGreen);
      g_par[i_p+1][1]->SetFillColorAlpha(kGreen, 0.5);
      g_par[i_p+1][1]->Draw("ce3");
  ...
  g_sig[1]->SetLineColor(kBlue);
  g_sig[1]->SetFillColorAlpha(kBlue, 0.5);
  g_sig[1]->Draw("ce3");
  ...
  double parmind[] = {0.6, 4.8e-3, 3.0964, 0.02, 0.036, 0.6, 1.8};
  double parmaxd[] = {0.8, 5.0e-3, 3.0970, 0.03, 0.05,  1.8, 2.5};
  ...
  for(int i_p = 0; i_p < n_p; i_p++) {
    ...
    for(int i_m = 1; i_m < 4; i_m++) {
      if(i_p == 1 || i_p == 5 || i_p == 6) {
	g_par[i_p][i_m]->SetLineColor(i_m+1);
	g_par[i_p][i_m]->SetMarkerColor(i_m+1);
	g_par[i_p][i_m]->Draw("p");
      }
      else {
	g_par[i_p][i_m]->SetLineColor(i_m+1);
	g_par[i_p][i_m]->SetMarkerColor(i_m+1);
	g_par[i_p][i_m]->SetFillColorAlpha(i_m+1, 0.5);
	g_par[i_p][i_m]->Draw("ce3");
      }
      leg->AddEntry(g_par[i_p][i_m], legT[i_m-1].c_str(), "pl");
    }
  ...
  for(int i_m = 1; i_m < 4; i_m++) {
    l_chi[i_m-1]->SetLineColor(i_m+1);
    l_chi[i_m-1]->Draw("lsame");
    leg->AddEntry(l_chi[i_m-1], legT[i_m-1].c_str(), "l");
  }
