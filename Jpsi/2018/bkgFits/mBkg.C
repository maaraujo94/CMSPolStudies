...
int DO_FILL = 0; // toggle for filling the histo (only if conditions change)
int DO_EXP = 1; // toggle for doing exp bkg (vs linear bkg)
...
void mBkg()
{
  ...
  for(int ip = 0; ip < nPtBins; ip++)
    h_d1d[ip] = new TH1D(Form("mH%.0f", ptBins[ip]), Form("2018 data M(#mu#mu) (%.0f < p_{T} < %.0f)",  ptBins[ip], ptBins[ip+1]), mbins, lowm, him);
  
  TH2D *h_d2d = new TH2D("h_d2d", "2018 data M(#mu#mu)", mbins, lowm, him, nPtBins, ptBins);
 
  cout << "all data mass histograms initialized" << endl;

  if(DO_FILL == 1) {
    // filling all the histos at once    
    // open and read the data tree
    TFile *fin1 = new TFile("/home/mariana/Documents/2020_PhD_work/CERN/CMSPolStudies/Jpsi/Store_data_codes/data18_cos.root");
    ...
    TFile *fout = new TFile("files/mStore_data.root", "recreate");
    for(int ip = 0; ip < nPtBins; ip++) {
      h_d1d[ip]->Write();	
    }
    fout->Close();
    ...
    TFile *fin = new TFile("files/mStore_data.root");
    for(int ip = 0; ip < nPtBins; ip++) {
  ...
  cout << "all mass histograms filled" << endl << endl;
  ...
  TF2 *f_cb = new TF2("f_cb", mmod_func, m_min[0], m_max[2], 25, 100, 5*nPtBins+6, 2);
  ...
  /* for(int i = 0; i < nPtBins; i++)
    cout << i << " " << (h_d1d[i]->GetMaximum()*2.)/f_cb->GetParameter(i+6+3*nPtBins) << " vs " << (h_d1d[i]->Integral()/5.)/f_cb->GetParameter(i+6+3*nPtBins)  << endl;
  */
