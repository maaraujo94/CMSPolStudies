...
void ltFit()
{
  ...
  TH1D **ltHist = new TH1D*[3];
  
  // open and read the histo store
  TFile *fin = new TFile("files/ltStore.root");
  string lbl_lt[] = {"SR", "LSB", "RSB"};
  for(int i = 0; i < 3; i++) {
    ltHist[i] = (TH1D*)fin->Get(Form("ltH%s", lbl_lt[i].c_str()));
    ltHist[i]->SetDirectory(0);
  }
  fin->Close();

  int tbins = ltHist[0]->GetNbinsX();
  double lowt = ltHist[0]->GetXaxis()->GetBinLowEdge(1);
  double hit = ltHist[0]->GetXaxis()->GetBinUpEdge(tbins);
  ...
  double pval[] = {1., 1., 0.8, 0, 1e-2, 3e-2, 0.35 };

  ofstream ftex;
  ftex.open("text_output/lt_fit.tex");
  ftex << "\\begin{tabular}{c|ccc}\n";
  for(int i = 0; i< 3; i++)
    ftex << Form(" & %s", lbl_lt[i].c_str());
  ftex << " \\\\\n";
  ftex << "\\hline\n";
  ftex << "$f_{NP}$ ";
  
  // cycle over 3 mass regions
  double fv[3];
  for(int i_r = 0; i_r < 3; i_r++) {
    // set fit parameters - fix shape after SR
    fitS->SetParameter(0, ltHist[i_r]->GetMaximum());
    fitS->SetParameter(1, ltHist[i_r]->GetMaximum()*5);
    for(int i_p = 2; i_p < 7; i_p++) 
      fitS->SetParameter(i_p, pval[i_p]);
    
    int fits = ltHist[i_r]->Fit(fitS, "R");
    ...
    double min_bin = ltHist[i_r]->GetXaxis()->FindBin(-(pr_lim-1e-6));
    double max_bin = ltHist[i_r]->GetXaxis()->FindBin(pr_lim-1e-6);
    double evt_all = ltHist[i_r]->Integral(min_bin, max_bin, "width");
    double fracNP = evt_NP / evt_all;
    fv[i_r] = fracNP;
    ftex << Form("& %.2f\\%% ", fracNP*100);

    cout << endl << endl << lbl_lt[i_r].c_str() << " " << fracNP << endl << endl;
    
    // PART 3 : PLOTTING AND STORING RESULTS
  
    // draw results
    c->SetLogy();
    ltHist[i_r]->SetMaximum(ltHist[i_r]->GetMaximum()*1.2);
    ltHist[i_r]->SetMinimum(ltHist[i_r]->GetMaximum()*1e-2);

    TH1F *fh = c->DrawFrame(lowPlot, ltHist[i_r]->GetMinimum(), hit, ltHist[i_r]->GetMaximum());
    ...
    fh->SetTitle(ltHist[i_r]->GetTitle());
    
    ltHist[i_r]->SetMarkerStyle(20);
    ltHist[i_r]->SetLineColor(kBlack);
    ltHist[i_r]->SetMarkerColor(kBlack);
    ltHist[i_r]->SetMarkerSize(.75);
    ltHist[i_r]->Draw("error same");
    ...
    TLine *lsig1 = new TLine(-pr_lim, ltHist[i_r]->GetMinimum(), -pr_lim, ltHist[i_r]->GetMaximum());
    lsig1->SetLineStyle(kDashed);
    lsig1->Draw("lsame");
    TLine *lsig2 = new TLine(pr_lim, ltHist[i_r]->GetMinimum(), pr_lim, ltHist[i_r]->GetMaximum());
    lsig2->SetLineStyle(kDashed);
    lsig2->Draw("lsame");
    TLine *lsig3 = new TLine(np_lim, ltHist[i_r]->GetMinimum(), np_lim, ltHist[i_r]->GetMaximum());
    lsig3->SetLineStyle(kDashed);
    lsig3->Draw("lsame");
    
    c->SaveAs(Form("plots/lifetime/fit_%s.pdf", lbl_lt[i_r].c_str()));
    c->Clear();
    
    // get the pulls and rel dev distribution
    double xv[tbins], pv[tbins], dv[tbins];
    for(int i_x = 0 ; i_x < tbins; i_x++) {
      xv[i_x] = ltHist[i_r]->GetBinCenter(i_x+1);
      double fitv = fitS->Eval(xv[i_x]);
      double datav = ltHist[i_r]->GetBinContent(i_x+1);
      double datau = ltHist[i_r]->GetBinError(i_x+1);
    ...
    TH1F *fp = c->DrawFrame(lowPlot, -pull_lim, hit, pull_lim);
    fp->SetXTitle("c#tau (mm)");
    fp->SetYTitle("pulls");
    fp->GetYaxis()->SetTitleOffset(1.3);
    fp->GetYaxis()->SetLabelOffset(0.01);
    fp->SetTitle(Form("Lifetime fit pulls (p_{T} integrated)"));
    ...
    TLine *psig1 = new TLine(-pr_lim, -pull_lim, -pr_lim, pull_lim);
    psig1->SetLineStyle(kDashed);
    psig1->Draw("lsame");
    TLine *psig2 = new TLine(pr_lim, -pull_lim, pr_lim, pull_lim);
    psig2->SetLineStyle(kDashed);
    psig2->Draw("lsame");
    TLine *psig3 = new TLine(np_lim, -pull_lim, np_lim, pull_lim);
    psig3->SetLineStyle(kDashed);
    psig3->Draw("lsame");
    
    c->SaveAs(Form("plots/lifetime/pulls_%s.pdf", lbl_lt[i_r].c_str()));
    c->Clear();
    ...
    fd->SetTitle(Form("Lifetime fit deviations (p_{T} integrated)"));
    ...
    c->SaveAs(Form("plots/lifetime/devs_%s.pdf", lbl_lt[i_r].c_str()));
    c->Clear();
  ...
  TGraph *g_frac = new TGraph(3, fv, fv);
  g_frac->SetName("g_fNP");
