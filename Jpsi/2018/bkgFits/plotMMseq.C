...
void plotMMseq()
{
  ...
  // do the plotting - n, alpha for fixed mu (MODEL 1)
  ...
  // do the plotting - alpha for fixed n, mu (MODEL 2)
  TH1F *f2 = c->DrawFrame(pt_min, parmin[6], pt_max, parmax[6]);
  f2->SetXTitle("p_{T} (GeV)");
  f2->SetYTitle(parax[6].c_str());
  f2->GetYaxis()->SetTitleOffset(1.8);
  f2->GetYaxis()->SetLabelOffset(0.01);
  f2->SetTitle("2018 #alpha (#mu, n constant)");

  c->SetLogy(0);
  
  // plot the points
  g_par[6][2]->SetLineColor(kBlack);
  g_par[6][2]->SetMarkerColor(kBlack);
  g_par[6][2]->SetMarkerStyle(20);
  g_par[6][2]->SetMarkerSize(.75);
  g_par[6][2]->Draw("p");
  
  TLine *l21 = new TLine(46, parmin[6], 46, parmax[6]);
  l21->SetLineColor(kBlack);
  l21->SetLineStyle(kDashed);
  l21->Draw();
  TLine *l22 = new TLine(66, parmin[6], 66, parmax[6]);
  ...
  // do the plotting - f, sigmas for fixed mu, n, alpha (MODEL 3)
  for(int i_p = 0; i_p < 4; i_p++) {
    ...
    fl->GetYaxis()->SetTitleOffset(1.8);
    fl->GetYaxis()->SetLabelOffset(0.01);
    fl->SetTitle(Form("2018 %s (#mu, n, #alpha constant)", partit[i_p].c_str()));
    ...
    // if we're plotting par 3, add par 4 (both sigmas in 1)
    if( i_p == 3) {      
      g_par[i_p+1][3]->SetLineColor(kBlue);
      g_par[i_p+1][3]->SetMarkerColor(kBlue);
      g_par[i_p+1][3]->SetMarkerStyle(24);
      g_par[i_p+1][3]->SetMarkerSize(.75);
      g_par[i_p+1][3]->Draw("p");
      
      TLegend *leg = new TLegend(0.75, 0.3, 0.9, 0.45);
      leg->SetTextSize(0.03);
      leg->AddEntry(g_par[i_p][3], "#sigma_{1}", "pl");
      leg->AddEntry(g_par[i_p+1][3], "#sigma_{2}", "pl");
      leg->Draw();
    }
    else if (i_p == 4) continue;
  ...
  // do the plotting - sigmas for fixed mu, n, alpha, f (MODEL 4)
  TH1F *f4 = c->DrawFrame(pt_min, parmin[3], pt_max, parmax[3]);
  f4->SetXTitle("p_{T} (GeV)");
  f4->SetYTitle(parax[3].c_str());
  f4->GetYaxis()->SetTitleOffset(1.8);
  f4->GetYaxis()->SetLabelOffset(0.01);
  f4->SetTitle(Form("2018 %s (#mu, n, alpha, f constant)", partit[3].c_str()));

  c->SetLogy(0);
  
  // free mode always plots points
  g_par[3][4]->SetLineColor(kBlack);
  g_par[3][4]->SetMarkerColor(kBlack);
  g_par[3][4]->SetMarkerStyle(20);
  g_par[3][4]->SetMarkerSize(.75);
  g_par[3][4]->Draw("p");

  g_par[3+1][4]->SetLineColor(kBlue);
  g_par[3+1][4]->SetMarkerColor(kBlue);
  g_par[3+1][4]->SetMarkerStyle(24);
  g_par[3+1][4]->SetMarkerSize(.75);
  g_par[3+1][4]->Draw("p");
  
  TLegend *leg4 = new TLegend(0.75, 0.3, 0.9, 0.45);
  leg4->SetTextSize(0.03);
  leg4->AddEntry(g_par[3][4], "#sigma_{1}", "pl");
  leg4->AddEntry(g_par[3+1][4], "#sigma_{2}", "pl");
  leg4->Draw();
  
  TLine *l41 = new TLine(46, parmin[3], 46, parmax[3]);
  l41->SetLineColor(kBlack);
  l41->SetLineStyle(kDashed);
  l41->Draw();
  TLine *l42 = new TLine(66, parmin[3], 66, parmax[3]);
  ...
  legc->AddEntry(l_chiN[2], "#mu, n", "l");
  legc->AddEntry(l_chiN[3], "#mu, n, #alpha", "l");
  legc->AddEntry(l_chiN[4], "#mu, n, #alpha, f", "l");
  legc->AddEntry(l_chiN[5], "#mu, n, #alpha, f, #sigma_{1,2}", "l");
  legc->Draw();
