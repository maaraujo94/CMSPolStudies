...
void store_ANdists()
{
  ...
  // 4 costh dists: PR data + NP data + 2 MC
  ...
  TFile *fin = new TFile("../../Store_data_codes/data16_cos.root");
  TTree *treeD = (TTree*)fin->Get("data_cos");
  TFile *fin2 = new TFile("../../Store_data_codes/MC16_cos.root");
  TTree *treeM1 = (TTree*)fin2->Get("MC_cos");
  TFile *fin2a = new TFile("../../Store_data_codes/MC16p_cos.root");
  ...
  for(int i = 0; i < m1aEvt; i++)
	...
	//if(mc_pt > 45 && mc_pt < 50) 
      ...
      if(mc_m > 3.0 && mc_m < 3.2 && abs(mc_y) < 1.2 && abs(mc_lt) < 0.005) {
	//if(mc_pt > 45 && mc_pt < 50) {
	  h_cos[3]->Fill(abs(cos(mc_th)));
      }
      
    }
  
  cout << "second MC filled" << endl;
