...
void fbkgProp_NP()
{
  ...
  TH1D *h_fbkg = new TH1D("h_fbkg", "2016 f_{Bg}", n_pt, ptBins);
  double ln = 10000;
  const int n_p = 2;
  ...
  fr1->SetTitle("2016 f_{Bg}^{NP} vs p_{T}");
  ...
  TH2D *h_fbkg2d = new TH2D("h_fbkg2d", "2016 f_{Bg}^{NP}", nBinsX, minX, maxX, n_pt, ptBins);
  for(int i_pt = 0; i_pt < n_pt; i_pt++) {
