...
void genDist()
{
  // location of all the stuff I got for the cos(theta) dimension
  string loc_th = "/home/mariana/Documents/2020_PhD_work/CERN/CMSPolStudies/Jpsi/Simult/PR_fit";
  string loc_NP = "/home/mariana/Documents/2020_PhD_work/CERN/CMSPolStudies/Jpsi/Simult/NP_fit";
  ...
  TFile *infL = new TFile(Form("%s/files/store_fL.root", loc_th.c_str()));
  ...
  for(int i_pt = 0; i_pt < nBinsY; i_pt++) {
    h_SB[i_pt] = new TH1D(Form("h_SB_%d", i_pt), Form("Run 2 SB #phi(%.0f < p_{T} < %.0f GeV)", yBins[i_pt], yBins[i_pt+1]), nBinsX, minX, maxX);
  ...
  h_SB[0]->SetMinimum(0.04);
  h_SB[0]->SetMaximum(0.06);
