...
void indFit()
{
  ...
  for(int i = 0; i < nBinsY; i++) {
    ...
    pHist[0][i]->GetXaxis()->SetTitle("#phi");
    ...
    lc.DrawLatex(-150, pHist[0][i]->GetMaximum()*0.8, Form("#beta^{prompt J/#psi} = %.3f #pm %.3f", parL[1][i], eparL[1][i]));
