...
void plotRes()
{
  ...
  double val[nBinspT], unc[nBinspT];
  for(int i = 0; i < nBinspT; i++) {
    val[i] = graph_lth[3]->GetY()[i] - graph_lthBase->GetY()[i];
    //set the unc as the diff in quadrature
    unc[i] = sqrt(abs(pow(graph_lthBase->GetEY()[i],2)-pow(graph_lth[3]->GetEY()[i],2)));

    cout << i << " " << graph_lthBase->GetEY()[i] << " " << graph_lth[3]->GetEY()[i] << endl;
  }
  TGraphErrors *g_lthD = new TGraphErrors(nBinspT, graph_lthBase->GetX(), val, graph_lthBase->GetEX(), unc);//graph_lthBase->GetEY());
