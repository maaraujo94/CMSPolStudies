...
void plotCosPars_all()
{
  ...
  for(int i_f = 0; i_f < n_fit; i_f++) { // cycle through fit format
	...
	fin->GetObject(Form("fit_%s%s", parex[i_f].c_str(), parlab[i_p].c_str()), g_par[i_f][i_inp][i_p]);	  
      }

      // get fit chi^2
      if(i_f < 2) {
	g_chi[i_f][i_inp] = new TGraph();
	fin->GetObject(Form("fit_%schiP", parex[i_f].c_str()), g_chi[i_f][i_inp]);
      }
      else {
	l_chi[i_inp] = new TLine();
	fin->GetObject(Form("fit_%schiP", parex[i_f].c_str()), l_chi[i_inp]);
      }
      fin->Close();
    }
  }
  ...
  for(int i_inp = 0; i_inp < n_inp; i_inp++) {
    ...
    for(int i = 0; i < n_fit-1; i++) {
      g_chi[i_inp][i]->SetMarkerStyle(20);
      g_chi[i_inp][i]->SetMarkerSize(1.5);
      g_chi[i_inp][i]->SetMarkerColor(col_s(i));
      g_chi[i_inp][i]->SetLineColor(col_s(i));
      g_chi[i_inp][i]->Draw("psame");
    }

    l_chi[i_inp]->SetLineColor(col_s(2));
    l_chi[i_inp]->Draw("lsame");
