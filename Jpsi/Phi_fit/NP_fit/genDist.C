...
void genDist()
{
  ...
  TFile *infL = new TFile("../../Simult/NP_fit/files/store_fL.root");
  ...
  for(int i_pt = 0; i_pt < nBinsY; i_pt++) {
    h_SB[i_pt] = new TH1D(Form("h_SB_%d", i_pt), Form("bkg^{NP} #phi (%.1f < p_{T} < %.1f GeV)", yBins[i_pt], yBins[i_pt+1]), nBinsX, minX, maxX);
