...
void bkgCosth()
{
  ...
  for(int i = 0; i < 2; i++) {
    dataHist_ab[i] = new TH2D(Form("dataH%d_ab", i), Form("Full Data (%s)", lbl[i].c_str()), 20, -180, 180, nPtBins, ptBins);
  }
  ...
  for(int i = 0; i < dEvt; i++)
	...
	if(data_m < 2.95 && data_m > 2.92) {
	  dataHist_ab[0]->Fill(data_phi, data_pt);
	}
	// RSB
	else if(data_m < 3.28 && data_m > 3.21) {
	  dataHist_ab[1]->Fill(data_phi, data_pt);
	}
      }
    }
  ...
  for(int i = 0; i < 2; i++) {
    dataHist_ab[i]->GetXaxis()->SetTitle("#phi_{HX}");
  ...
  for(int i = 0; i < 2; i++) {
    ratioHist_ab[i] = new TH2D(Form("ratioH%d_ab", i), Form("Full Data/MC (%s)", lbl[i].c_str()), 20, -180, 180, nPtBins, ptBins);
