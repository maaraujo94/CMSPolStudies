...
void newMCmass_0()
{
  ...
  for(int ip = 0; ip < nPtBins; ip++) {
    h_m1d[ip] = new TH1D(Form("mH%.0f" , ptBins[ip]), Form("Full MC M(#mu#mu) (%.0f < p_{T} < %.0f)",  ptBins[ip], ptBins[ip+1]), mbins, lowm, him);
    h_m1d[ip]->Sumw2();
  }
  ...
  if(DO_FILL == 1) {
    ...
    tree2->SetBranchAddress("muonPPt", &mP_pt);
    tree2->SetBranchAddress("muonPEta", &mP_eta);
    tree2->SetBranchAddress("muonMPt", &mM_pt);
    tree2->SetBranchAddress("muonMEta", &mM_eta);
