...
void plotDMPars()
{
  ...
  for(int i_m = 0; i_m < n_m; i_m++) {
    ...
    TFile *fin = new TFile(Form("files/mfit_%s.root", modn[i_m].c_str()));
    int n_pv = n_p;
  ...
  // also plot the chisquare comparison
  TH1D *h_chi = new TH1D("h_chi", "", n_m, 0, n_m);
  double chi2[n_m], ndf[n_m];
  for(int i_m = 0; i_m < n_m; i_m++) {
    TFile *fin = new TFile(Form("files/mfit_%s.root", modn[i_m].c_str()));
    chi2[i_m] = ((TFitResult*)fin->Get("fitres"))->Chi2();
    ndf[i_m] = (double)((TFitResult*)fin->Get("fitres"))->Ndf();
    h_chi->Fill(legn[i_m].c_str(), chi2[i_m]/ndf[i_m]);
    fin->Close();
  }

  h_chi->SetStats(0);
  h_chi->GetXaxis()->SetTitle("fit");
  h_chi->GetYaxis()->SetTitle("#chi^{2}/ndf");
  h_chi->GetYaxis()->SetTitleOffset(1.3);
  h_chi->GetYaxis()->SetRangeUser(0, 5);
  h_chi->LabelsDeflate("X");
  h_chi->SetMarkerStyle(20);
  h_chi->SetMarkerSize(0.75);
  ...
  l.SetTextSize(0.03);
  for(int i_m = 0; i_m < n_m; i_m++) {
    l.DrawLatex(i_m+0.2, h_chi->GetBinContent(i_m+1)+0.15, Form("#bf{%.0f/%.0f}", chi2[i_m], ndf[i_m]));
  }
  
  c->SaveAs("plots/mass/par_chiN.pdf");
