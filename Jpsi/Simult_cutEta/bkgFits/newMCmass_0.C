...
void newMCmass_0()
{
  ...
  if(DO_FILL == 1) {
    ...
    for(int i = 0; i < mEvt; i++)
      {
	tree2->GetEntry(i);
	if(mc_pt > 46 && mc_pt < 66 && abs(mc_lt) < 0.005 && (abs(mPEta) < 0.2 || abs(mPEta) > 0.3) && (abs(mMEta) < 0.2 || abs(mMEta) > 0.3)) {
	  for(int i_p = 0; i_p < nPtBins; i_p++)
	    if(mc_pt > ptBins[i_p] && mc_pt < ptBins[i_p+1])
	      h_m1d[i_p]->Fill(mc_m);
	}
    ...
    for(int i = 0; i < mEvt; i++)
      {
	tree3->GetEntry(i);
	if(mc_pt > 66 && mc_pt < ptBins[nPtBins] && abs(mc_lt) < 0.005 && (abs(mPEta) < 0.2 || abs(mPEta) > 0.3) && (abs(mMEta) < 0.2 || abs(mMEta) > 0.3)) {
	  for(int i_p = 0; i_p < nPtBins; i_p++)
	    if(mc_pt > ptBins[i_p] && mc_pt < ptBins[i_p+1])
	      h_m1d[i_p]->Fill(mc_m);
	}
