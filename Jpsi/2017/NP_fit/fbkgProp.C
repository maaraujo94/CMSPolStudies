...
TH1D **h_d1d = new TH1D*[n_pt];
  ...
  TH1D *h_fbkg = new TH1D("h_fbkg", "2017 f_{bkg}^{NP}", n_pt, ptBins);
  double ln = 10000;
  const int n_p = 2;
  ...
  fr1->SetTitle("2017 f_{bkg}^{NP} vs p_{T}");
  ...
  TH2D *h_fbkg2d = new TH2D("h_fbkg2d", "2017 f_{bkg}^{NP}", nBinsX, minX, maxX, nBinsY, yBins);
  ...
  h_fbkgpt->SetTitle("2017 f_{bkg}^{NP} vs p_{T}");
  h_fbkgpt->SetStats(0);
