...
void fitMassSigma()
{
  ...
  TFile *fin2 = new TFile("../Store_data_codes/MCh17_cos.root");
  TTree *tree2 = (TTree*)fin2->Get("MC_cos");
  TFile *fin3 = new TFile("../Store_data_codes/MCvh17_cos.root");
  TTree *tree3 = (TTree*)fin3->Get("MC_cos");

  int nEvt, perc;
  double y, mass, pt, lt, lterr;

  // low-pT MC part
  nEvt = tree1->GetEntries();
  perc = nEvt/100;
  tree1->SetBranchAddress("Mass", &mass);
  tree1->SetBranchAddress("Rap", &y);
  tree1->SetBranchAddress("dimPt", &pt);
  tree1->SetBranchAddress("lt", &lt);
  tree1->SetBranchAddress("lterr", &lterr);

  // fill histograms
  for(int i_evt = 0; i_evt < nEvt; i_evt++) {
    tree1->GetEntry(i_evt);
    if(mass > mMin && mass < mMax && pt > pt_min[0] && pt < 46 && abs(lt/lterr) < 2.5) {
  ...
  tree2->SetBranchAddress("Rap", &y);
  tree2->SetBranchAddress("dimPt", &pt);
  tree2->SetBranchAddress("lt", &lt);
  tree2->SetBranchAddress("lterr", &lterr);

  // fill histograms
  for(int i_evt = 0; i_evt < nEvt; i_evt++) {
    tree2->GetEntry(i_evt);
    if(mass > mMin && mass < mMax && pt > 46 && pt < 66 && abs(lt/lterr) < 2.5) {
      for(int i_pt = 0; i_pt < pt_bins; i_pt++) {
	if(pt > pt_min[i_pt] && pt < pt_max[i_pt]) {
	  for(int i_y = 0; i_y < y_bins; i_y++) { 
	    if(y > y_min[i_y] && y < y_max[i_y]) {
  ...
  perc = nEvt/100;
  tree3->SetBranchAddress("Mass", &mass);
  tree3->SetBranchAddress("Rap", &y);
  tree3->SetBranchAddress("dimPt", &pt);
  tree3->SetBranchAddress("lt", &lt);
  tree3->SetBranchAddress("lterr", &lterr);

  // fill histograms
  for(int i_evt = 0; i_evt < nEvt; i_evt++) {
    tree3->GetEntry(i_evt);
    if(mass > mMin && mass < mMax && pt > 66 && pt < pt_max[pt_bins-1] && abs(lt/lterr) < 2.5) {
      for(int i_pt = 0; i_pt < pt_bins; i_pt++) {
	if(pt > pt_min[i_pt] && pt < pt_max[i_pt]) {
	  for(int i_y = 0; i_y < y_bins; i_y++) { 
	    if(y > y_min[i_y] && y < y_max[i_y]) {
  ...
  for(int i_pt = 0; i_pt < pt_bins; i_pt++) {
    ...
    for(int i_y = 0; i_y < y_bins; i_y++) {
      ...
      double n_est = h_m[i_pt][i_y]->Integral() / 50.;
      f3->SetParameters(n_est, 3.097, 0.03);
      ...
      c->SaveAs(Form("plots_massFit/mass_pT%d_y%d.pdf", i_pt+1, i_y+1));
      c->Clear();
    }
  ...
  c->SaveAs("plots_massFit/sigma_fit.pdf");
  c->Clear();
  ...
  c->SaveAs("plots_massFit/sigma_cont.pdf");
  c->Clear();
  ...
  c->SaveAs("plots_massFit/mu_fit.pdf");
  c->Clear();
  
  // output tex and txt file with fit results
  ofstream tex;
  tex.open("plots_massFit/fit_sig.tex");
  ...
  ofstream txt;
  txt.open("plots_massFit/fit_sig.txt");
