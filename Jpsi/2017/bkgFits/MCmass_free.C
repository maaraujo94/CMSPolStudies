...
void MCmass_free()
{
  ...
  for(int ip = 0; ip < nPtBins; ip++) {
    mHist[ip] = new TH1D(Form("mH%.0f", ptBins[ip]), Form("2017 MC M(#mu#mu) (%.0f < p_{T} < %.0f GeV)", ptBins[ip], ptBins[ip+1]), mbins, lowm, him);
  }
  ...
  cout << "all J/psi MC mass histograms filled" << endl << endl;
  ...
  for(int i_pt = 0; i_pt < nPtBins; i_pt++) {
    ...
    mHist[i_pt]->SetMaximum(mHist[i_pt]->GetMaximum()*1.2);
    mHist[i_pt]->SetMinimum(mHist[i_pt]->GetMaximum()*5e-4);
    mHist[i_pt]->SetStats(0);
    mHist[i_pt]->GetYaxis()->SetTitle(Form("Events per %.0f MeV", (him-lowm)/mbins*1000));
    mHist[i_pt]->GetYaxis()->SetTitleOffset(1.4);
    mHist[i_pt]->GetXaxis()->SetTitle(Form("M(#mu#mu) (GeV)"));
    mHist[i_pt]->SetMarkerStyle(20);
    mHist[i_pt]->SetMarkerSize(0.75);
    mHist[i_pt]->SetMarkerColor(kBlack);
