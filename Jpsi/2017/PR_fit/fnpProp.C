...
void fnpProp()
{
  ...
  TH1D *h_fnp_psi = new TH1D("h_fnp_psi", "2017 f_{NP}", nPtBins, ptBins);
  ...
  TF1 *fexp_bkg = new TF1("fexp_bkg", "pos_exp(x,[0])", 5*lowt, 5*hit);
  ...
  TF1 *fexp_nbkg = new TF1("fexp_nbkg", "[0]*pos_exp(x,[1])", 5*lowt, 5*hit);

  // get corrected N_bkg, then get fNP_bkg
  double b_pars[8];
  TH1D *h_fnp_bkg = new TH1D("h_fnp_bkg", "2017 f_{NP_{bkg}}", nPtBins, ptBins);
  ...
  for(int i_pt = 0; i_pt < nPtBins; i_pt++) {
    ...
    for(int i_p = 7; i_p < 8; i_p++) // bkg pars
      b_pars[i_p] = fitres->Parameter((i_p+3)*nPtBins + di[i_p+3] * i_pt);

    // scale N_bkg par
    for(int i_p = 0; i_p < 8; i_p++) 
      fNP_bkg->SetParameter(i_p, b_pars[i_p]);
    fexp_nbkg->SetParameters(fitres->Parameter(9*nPtBins + i_pt), b_pars[7]);
  ...
  TH2D *h_fnp2d = new TH2D("h_fnp2d", "2017 f_{NP}", nBinsX, minX, maxX, nPtBins, ptBins);
  for(int i_pt = 0; i_pt < nPtBins; i_pt++) {
