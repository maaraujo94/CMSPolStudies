...
void plot_ANdists()
{
  ...
  lcpt.DrawLatex(45, yl, "#bf{2017 J/#psi}");
  ...
  for(int i = 0; i < 5; i++) {
    ...
    if(i == 0) {
      ...
      h_pT[i]->Scale(h_pT[0]->Integral(iBin[i-1],fBin[i-1]) / h_pT[i]->Integral(iBin[i-1],fBin[i-1]));
      h_pT[i]->Draw("histo same");
    }
  }
  
  TLatex lcpts;
  yl = getPos(h_pT[0]->GetMinimum(), h_pT[0]->GetMaximum(), 0.91, 1);
  lcpts.SetTextSize(0.04);
  lcpts.DrawLatex(45, yl, "#bf{2017 J/#psi}");
  lcpts.SetTextColor(colpt[0]);
  lcpts.DrawLatex(105, yl, "#bf{PRS data}");
  yl = getPos(h_pT[0]->GetMinimum(), h_pT[0]->GetMaximum(), 0.83, 1);
  lcpts.DrawLatex(105, yl, "#bf{Prompt MC samples}");
  lcpts.SetTextColor(colpt[1]);
  yl = getPos(h_pT[0]->GetMinimum(), h_pT[0]->GetMaximum(), 0.76, 1);
  lcpts.DrawLatex(120, yl, "#bf{[25,46] GeV}");
  lcpts.SetTextColor(colpt[2]);
  yl = getPos(h_pT[0]->GetMinimum(), h_pT[0]->GetMaximum(), 0.69, 1);
  lcpts.DrawLatex(120, yl, "#bf{[40,52] GeV}");
  lcpts.SetTextColor(colpt[3]);
  yl = getPos(h_pT[0]->GetMinimum(), h_pT[0]->GetMaximum(), 0.62, 1);
  lcpts.DrawLatex(120, yl, "#bf{> 46 GeV}");
  lcpts.SetTextColor(colpt[4]);
  yl = getPos(h_pT[0]->GetMinimum(), h_pT[0]->GetMaximum(), 0.55, 1);
  lcpts.DrawLatex(120, yl, "#bf{> 66 GeV}");

  ptL->Draw();

  c->SaveAs(Form("plots/ANdists/pt_scale.pdf"));
  c->Clear();
  ...
  TLegend *legcy = new TLegend(0.6, 0.79, 0.9, 0.915);
  ...
  yl = getPos(h_y[0]->GetMinimum(), h_y[0]->GetMaximum(), 0.84, 1);
  lcy.DrawLatex(-1, yl, "#bf{2017 J/#psi}");
  ...
  TLegend *legcys = new TLegend(0.575, 0.55, 0.875, 0.675);
  ...
  lcys.DrawLatex(-1, yl, "#bf{2017 J/#psi}");
  lcys.SetTextColor(coly[0]);
  yl = getPos(h_y[0]->GetMinimum(), h_y[0]->GetMaximum(), 0.66, 1);
  lcys.DrawLatex(-1, yl, "#bf{[25,45] GeV}");
  lcys.SetTextColor(coly[1]);
  yl = getPos(h_y[0]->GetMinimum(), h_y[0]->GetMaximum(), 0.58, 1);
  lcys.DrawLatex(-1, yl, "#bf{[45,50] GeV}");
  lcys.SetTextColor(coly[2]);
  yl = getPos(h_y[0]->GetMinimum(), h_y[0]->GetMaximum(), 0.5, 1);
  lcys.DrawLatex(-1, yl, "#bf{[50,70] GeV}");
  lcys.SetTextColor(coly[3]);
  yl = getPos(h_y[0]->GetMinimum(), h_y[0]->GetMaximum(), 0.42, 1);
  ...
  lcyr.DrawLatex(-1.1, yl, "#bf{2017 J/#psi}");
  ...
  for(int i = 0; i < 4; i++) {
    ...
    h_lt[i]->SetMinimum(3e2);
    h_lt[i]->SetMaximum(2e6);
  ...
  lclt.DrawLatex(150, yl, "#bf{2017 J/#psi}");
  ...
  h_lt[4]->SetMinimum(3e2);
  h_lt[4]->SetMaximum(2e6);
  ...
  yl = getPos(h_lt[0]->GetMinimum(), h_lt[0]->GetMaximum(), 0.9, 1);
  lclts.DrawLatex(150, yl, "#bf{2017 J/#psi}");
  ...
  lcm.DrawLatex(2.95, 0.074, "#bf{2017 J/#psi}");
