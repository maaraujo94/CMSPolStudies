void plotCosPars2d()
{
  ...
  double parmin[] = {3e-4, -2, -2};
  double parmax[] = {5e-3, 2, 2};
  ...
  for(int i_inp = 0; i_inp < n_inp; i_inp++) {
    ...
    TFile *fin = new TFile(Form("files/%slin_fitres.root", lbl[i_inp].c_str()));
	...
	g_par[i]->Draw("psame");
      ...
      c->SaveAs(Form("plots/%slin/par_%s.pdf", lbl[i_inp].c_str(), parlab[i].c_str()));
      c->Clear();
    }
  }
