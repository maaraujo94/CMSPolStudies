...
void plotRes()
{
  ...
  TFile *fIndz = new TFile("../PR_fit/files/finalFitRes_ltpz.root");
  ...
  // lth is immediate
  TFile *find_th = new TFile("../../Simult/PR_fit/files/finalFitRes.root");
  TGraphErrors *graph_lth = (TGraphErrors*)find_th->Get(Form("graph_lambda_J"));
  find_th->Close();
  // lphi need to get from beta
  TFile *find_ph = new TFile("../../Phi_fit/PR_fit/files/finalFitRes.root");
  TGraphErrors *graph_beta = (TGraphErrors*)find_ph->Get(Form("graph_lambda_J"));
  find_ph->Close();
  ...
  c->SaveAs("lth_HX_comp.pdf");
  c->Clear();

  // now lambda_phi
  TH1F *flph = c->DrawFrame(ptBins[0]-5, -1, ptBins[nPtBins], 1);
  flph->SetXTitle("p_{T} (GeV)");
  flph->SetYTitle("#lambda_{#phi}^{PR}");
  flph->GetYaxis()->SetTitleOffset(1.3);
  flph->GetYaxis()->SetLabelOffset(0.01);
  flph->SetTitle("");
    
  graph_lPR[1]->SetLineColor(col[0]);
  graph_lPR[1]->SetMarkerStyle(20);
  graph_lPR[1]->SetMarkerSize(.5);
  graph_lPR[1]->SetMarkerColor(col[0]);
  graph_lPR[1]->Draw("p same");

  graph_lPRz[1]->SetLineColor(col[0]);
  graph_lPRz[1]->SetMarkerStyle(24);
  graph_lPRz[1]->SetMarkerSize(.5);
  graph_lPRz[1]->SetMarkerColor(col[0]);
  graph_lPRz[1]->SetLineStyle(kDashed);
  graph_lPRz[1]->Draw("p same");
  ...
  legph->AddEntry(graph_lPR[1], "#lambda_{#phi}^{HX} (2d)", "pl");
  legph->AddEntry(graph_lPRz[1], "#lambda_{#theta}^{HX} (2d w/o #lambda_{#theta#phi})", "pl");
  legph->AddEntry(graph_lph, "#lambda_{#phi}^{HX} (1d)", "pl");
  legph->Draw();

  c->SaveAs("lph_HX_comp.pdf");
  c->Clear();
