...
void getCuts_MCcomp()
{
  string name[2][2] = {{"filtered-all-psi-mc-LOCAL17-highpt.root", "filtered-all-psi-mc-LOCAL17-veryhighpt.root"},
		       {"filtered-all-psi-mc-LOCAL18-highpt.root", "filtered-all-psi-mc-LOCAL18-veryhighpt.root"}};
  string lbl[] = {"hpt", "vhpt"};
  TFile *outE = new TFile("Jpsi_MCh_comp.root", "recreate");
  outE->Close();
  ...
  for(int i_y = 0; i_y < 2; i_y++) { 
	...
	TH1D *h_muPpT  = new TH1D(Form("h%d_muPpT_%s", i_y+7, lbl[i].c_str()),  "MC muon pT",      90, 0, 90);
	TH1D *h_muNpT  = new TH1D(Form("h%d_muNpT_%s", i_y+7, lbl[i].c_str()),  "muonN pT",        90, 0, 90);
	...
	TH1D *h_JPt    = new TH1D(Form("h%d_JPt_%s", i_y+7, lbl[i].c_str()),    "MC dimuon pT",    140, 78, 92);
	...
	for(int i = 0; i < nEvt; i++) {
	  tree->GetEntry(i);
	  if(mumu_p4->Pt() > 80 && mumu_p4->Pt() < 90)
	    {
	...
	TFile *outfile = new TFile("Jpsi_MCh_comp.root", "update");
	h_muPpT->Write();
	h_muNpT->Write();
	h_muPEta->Write();
	h_muNEta->Write();
