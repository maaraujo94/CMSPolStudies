...
void ang_MC16()
{
  ...
  // 2016 tree
  TFile *fin6 = new TFile("/eos/user/m/maaraujo/JpsiRun2/MC/filtered-all-psi-mc-UL16.root");
  ...
  int mEvt = tree6->GetEntries();
  int perc = mEvt / 100;

  TFile *fout6 = new TFile("MC16_cos.root", "recreate");
  TTree *newtree6 = new TTree("MC_cos", "");
  ...
  for(int i = 0; i < mEvt; i++) {
    ...
    if((i+1)%perc == 0) cout << (i+1)/perc << "% done with 2016 MC" << endl; 
  }
  ...
  // 2016 tree (pre)
  TFile *fin6p = new TFile("/eos/user/m/maaraujo/JpsiRun2/MC/filtered-all-psi-mc-UL16pre.root");
  TTree *tree6p = (TTree*)fin6p->Get("jpsitree");
  
  tree6p->SetBranchAddress("muP_p4", &muP_p4);
  tree6p->SetBranchAddress("muM_p4", &muM_p4);
  tree6p->SetBranchAddress("mumu_p4", &mumu_p4);
  tree6p->SetBranchAddress("ctpv", &ct);
  tree6p->SetBranchAddress("ctpv_error", &ctErr);

  mEvt = tree6p->GetEntries();
  perc = mEvt / 100;

  TFile *fout6p = new TFile("MC16p_cos.root", "recreate");
  TTree *newtree6p = new TTree("MC_cos", "");

  newtree6p->Branch("theta", &th);
  newtree6p->Branch("phi", &phi);
  newtree6p->Branch("dimPt", &mmPt);
  newtree6p->Branch("lt", &lt);
  newtree6p->Branch("lterr", &lterr);
  newtree6p->Branch("Mass", &mass);
  newtree6p->Branch("Rap", &rap);
  newtree6p->Branch("muonPPt", &mPPt);
  newtree6p->Branch("muonPEta", &mPEta);
  newtree6p->Branch("muonMPt", &mMPt);
  newtree6p->Branch("muonMEta", &mMEta);
  newtree6p->Branch("DeltaR", &dR);
  ...
  for(int i = 0; i < mEvt; i++) {
    ...
    if((i+1)%perc == 0) cout << (i+1)/perc << "% done with 2016 MC (pre)" << endl; 
  }
