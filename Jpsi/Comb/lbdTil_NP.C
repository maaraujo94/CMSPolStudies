void lbdTil_NP()
{
  ...
  TGraphErrors *g_cos = (TGraphErrors*)fcos->Get("graph_lambda_NP");
  fcos->Close();
  // gives beta = f(lth,lphi)
  TFile *fphi = new TFile("../Phi_fit/PR_fit/files/finalFitRes.root");
  TGraphErrors *g_phi = (TGraphErrors*)fphi->Get("graph_lambda_NP");
  fphi->Close();
  ...
  TH1F *fl = c->DrawFrame(20, -0.5, 120, 0.5);
  ...
  c->SaveAs("plots/lbd_tilde_NP.pdf");
  c->Destructor();
}
