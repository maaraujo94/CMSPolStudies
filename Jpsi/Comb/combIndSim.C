...
void combIndSim()
{
  ...
  for(int i_l = 0; i_l < n_lbl; i_l++) {
    ...
    if(i_l==0) fl->SetTitle("prompt J/#psi #lambda_{#theta}");
    else fl->SetTitle("non-prompt J/#psi #lambda_{#theta}");
    ...
    if(i_l==0) fl1->SetTitle("prompt J/#psi #lambda_{#theta}");
    else fl1->SetTitle("non-prompt J/#psi #lambda_{#theta}");
