...
void ang_MCNP()
{
  ...
  // 2018 tree
  TFile *fin8 = new TFile("/eos/user/m/maaraujo/Psi2SRun2/filtered-all-psi2s-mc-LOCAL18-bplus.root");
  ...
  TFile *fout8 = new TFile("MCNP18_cos.root", "recreate");
  ...
  for(int i = 0; i < mEvt; i++) {
    ...
    if((i+1)%perc == 0) cout << (i+1)/perc << "% done with 2018 NP MC" << endl; 
  }
