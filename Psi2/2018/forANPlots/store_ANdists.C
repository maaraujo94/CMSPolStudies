...
void store_ANdists()
{
  ...
  // three pT dists: PRSR data + the 2 MC
  ...
  TH1D **h_m = new TH1D*[5]; 
  for(int i = 0; i < 5; i++)
    h_m[i] = new TH1D(Form("h_m%d", i), Form("M distributions"), 130, 3.35, 4.0);
  ...
  for(int i = 0; i < dEvt; i++)
	...
	if(data_pt > 25 && data_pt < 46) 
	  h_y[0]->Fill(data_y);
	else if(data_pt > 46 && data_pt < 120) 
	  h_y[1]->Fill(data_y);
      }
	...
	if(data_pt > 25 && data_pt < 120) 
      ...
      if(data_m > m_min[1] && data_m < m_max[1] && abs(data_y) < 1.2 && data_lt > 0.01 && data_lt < 0.05) {
	if(data_pt > 25 && data_pt < 46) 
  ...
  // store the cos dists
  string lbl_cos[] = {"lowPtPR", "highPtPR", "fullPR", "lowPtNP", "highPtNP", "fullNP", "lowPtMC", "highPtMC", "fullMC"};
  for(int i = 0; i < 9; i++) {
