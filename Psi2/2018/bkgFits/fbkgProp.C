...
void fbkgProp()
{
  ...
  TFile *inBG = new TFile("files/mfit_1.root");
  int n_par = 2;
  ...
  TH1D *h_fbkg = new TH1D("h_fbkg", "2018 f_{Bg}", nPtBins, ptBins);
  ...
  TH2D *h_fbkg2d = new TH2D("h_fbkg2d", "2018 f_{Bg}", nBinsX, minX, maxX, nPtBins, ptBins);
  for(int i_pt = 0; i_pt < nPtBins; i_pt++) {
