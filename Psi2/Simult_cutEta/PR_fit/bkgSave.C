...
void bkgSave()
{
  ...
  if(DO_LT == 1) {
   ...
   const int nPtBins = 7;
	...
	if(data_pt > ptBins[0] && data_pt < ptBins[nPtBins] && data_m < m_max && data_m > m_min && abs(data_y) < 1.2 && (abs(mPEta) < 0.2 || abs(mPEta) > 0.3) && (abs(mMEta) < 0.2 || abs(mMEta) > 0.3)) {
  ...
  if(DO_MASS == 1) {
    ...
    for(int i = 0; i < dEvt; i++)
      {
	tree1->GetEntry(i);
	if(data_pt > ptBins[0] && data_pt < ptBins[nPtBins] && abs(data_lt) < 0.005 && abs(data_y) < 1.2 && (abs(mPEta) < 0.2 || abs(mPEta) > 0.3) && (abs(mMEta) < 0.2 || abs(mMEta) > 0.3)) {
	  for(int i_p = 0; i_p < nPtBins; i_p++)
	    if(data_pt > ptBins[i_p] && data_pt < ptBins[i_p+1])
