...
void newMCmass_0()
{
  ...
  if(DO_FILL == 1) {
    ...
    TFile *fin1 = new TFile("/home/mariana/Documents/2020_PhD_work/CERN/CMSPolStudies/Psi2/Store_data_codes/MCS_cos.root");
    TTree *tree1 = (TTree*)fin1->Get("MC_cos");
    TFile *fin3 = new TFile("/home/mariana/Documents/2020_PhD_work/CERN/CMSPolStudies/Psi2/Store_data_codes/MCvhS_cos.root");
    ...
    for(int i = 0; i < mEvt; i++)
      {
	tree1->GetEntry(i);
	if(mc_pt > ptBins[0] && mc_pt < 46 && abs(mc_lt) < 0.005 && (abs(mPEta) < 0.2 || abs(mPEta) > 0.3) && (abs(mMEta) < 0.2 || abs(mMEta) > 0.3)) {
    ...
    for(int i = 0; i < mEvt; i++)
      {
	tree3->GetEntry(i);
	if(mc_pt > 46 && mc_pt < ptBins[nPtBins] && abs(mc_lt) < 0.005 && (abs(mPEta) < 0.2 || abs(mPEta) > 0.3) && (abs(mMEta) < 0.2 || abs(mMEta) > 0.3)) {
	  for(int i_p = 0; i_p < nPtBins; i_p++)
	    if(mc_pt > ptBins[i_p] && mc_pt < ptBins[i_p+1])
	      h_m1d[i_p]->Fill(mc_m);
	}
  ...
  for(int i_pt = 0; i_pt < nPtBins; i_pt++) {
    ...
    fl->SetTitle(Form("MC #psi(2S) Pulls (%.0f < p_{T} < %.0f)", ptBins[i_pt], ptBins[i_pt+1]));
