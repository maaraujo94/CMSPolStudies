// macro to compare fit parameters w/ two diff bkg functions
void plotDMPars_all()
{
  // aux arrays
  int pc[] = {kBlue, kGreen};
  int sigc[] = {kBlack, kViolet};
  const int n_p = 10, n_m = 2;
  string modn[] = {"../../PR_fit/", ""};
  string legn[] = {"no G", "with G"};

  string parlab[] = {"f", "NS", "mu", "sig1", "sig2", "n", "alpha", "fBG", "fG", "sigG"};
  string partit[] = {"f", "N_{SR}", "#mu", "#sigma", "#sigma_{2}", "n", "#alpha", "f_{bkg}", "f_{G}", "#sigma_{G}"};
  string parax[] = {"f (%)", "N_{SR} per 1 GeV", "#mu (MeV)", "#sigma_{1} (MeV)", "#sigma_{2} (MeV)", "n", "#alpha", "f_{bkg} (%)", "f_{G} (%)", "#sigma_{G} (MeV)"};
  
  double parmin[] = {0,   3e-1, 3650, 0,   32, 1.0, 2.0,  0.,  0,   0};
  double parmax[] = {100, 3e2,  3750, 100, 46, 1.4, 2.25, 100, 100, 100};
  ...
  double mult[] = {100., 1., 1e3, 1e3, 1e3, 1., 1., 100., 100., 1e3};
  for(int i_m = 0; i_m < n_m; i_m++) {
  ...
  for(int i_p = 0; i_p < 8; i_p++) {
	...
	g_par_s[i_m][i_p]->Draw("ce3");
      ...
      if( i_p == 0) {      
	g_par_s[i_m][8]->SetMarkerStyle(22);
	g_par_s[i_m][8]->SetLineColor(kBlack);
	g_par_s[i_m][8]->SetMarkerColor(kBlack);
	g_par_s[i_m][8]->SetFillColorAlpha(kBlack, 0.5);
	g_par_s[i_m][8]->Draw("ce3");
      }
	...
	g_par_s[i_m][9]->SetMarkerStyle(29);
	g_par_s[i_m][9]->SetMarkerColor(kRed);
	g_par_s[i_m][9]->SetLineColor(kRed);
	g_par_s[i_m][9]->SetFillColorAlpha(kRed, 0.5);
	g_par_s[i_m][9]->Draw("ce3");
      }
    }

    if(i_p == 0) {
      leg->AddEntry(g_par_s[0][0], "f_{CB1}", "l");
      leg->AddEntry(g_par_s[0][8], "f_{G}", "l");
      leg->Draw();
    }
    else if(i_p == 3) {
      leg->AddEntry(g_par_s[0][3], "#sigma_{1}", "l");
      leg->AddEntry(g_par_s[0][4], "#sigma_{2}", "l");
      leg->AddEntry(g_par_s[0][9], "#sigma_{G}", "l");
      leg->Draw();
    }
