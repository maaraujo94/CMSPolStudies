...
void newDatamass_1()
{
  ...
  for(int i = 0; i < nPtBins; i++) {
    ...
    h_d1d[i]->SetTitle(Form("2017 data M(#mu#mu) (%.1f < p_{T} < %.1f GeV)", ptBins[i], ptBins[i+1]));
  }
  ...
  for(int i_pt = 0; i_pt < nPtBins; i_pt++) {
    ...
    c->SaveAs(Form("plots/mass/fit_1/fit_pt%d.pdf", i_pt));
    c->Clear();
    ...
    c->SaveAs(Form("plots/mass/fit_1/fit_pt%d_log.pdf", i_pt));
    c->Clear();
    ...
    c->SaveAs(Form("plots/mass/fit_1/pulls_pt%d.pdf", i_pt));
    c->Clear();
    ...
    c->SaveAs(Form("plots/mass/fit_1/devs_pt%d.pdf", i_pt));
    c->Clear();
  }
  
  // storing the free parameters
  TFile *fout = new TFile("files/mfit_1.root", "recreate");
