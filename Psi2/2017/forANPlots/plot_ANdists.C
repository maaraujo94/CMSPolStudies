...
void plot_ANdists()
{
  ...
  lcpt.DrawLatex(35, yl, "#bf{2017 #psi(2S)}");
  ...
  // scale MC to data for shape comparison
  int iBin[] = {1, 14, 25};
  for(int i = 1; i < 4; i++) {
    cout << i <<  " " << iBin[i-1] << " "  << h_pT[0]->GetXaxis()->GetBinLowEdge(iBin[i-1]) << endl;
    cout << h_pT[0]->Integral(iBin[i-1],200) << " / " << h_pT[i]->Integral(iBin[i-1],200) << endl;
  }
  ...
  for(int i = 0; i < 4; i++) {
    ...
    if(i == 0) {
      ...
      h_pT[i]->Scale(h_pT[0]->Integral(iBin[i-1],200)/h_pT[i]->Integral(iBin[i-1],200));
      h_pT[i]->Draw("histo same");
    }
  }
  ...
  lcpts.DrawLatex(35, yl, "#bf{2017 #psi(2S)}");
  lcpts.SetTextColor(colpt[0]);
  lcpts.DrawLatex(90, yl, "#bf{PRS data}");
  yl = getPos(h_pT[0]->GetMinimum(), h_pT[0]->GetMaximum(), 0.83, 1);
  lcpts.SetTextColor(colpt[1]);
  lcpts.DrawLatex(90, yl, "#bf{Prompt MC sample}");
  ...
  lcy.DrawLatex(-1, yl, "#bf{2017 #psi(2S)}");
  ...
  TLegend *legcys = new TLegend(0.575, 0.425, 0.875, 0.55);
  ...
  lcys.DrawLatex(-1, yl, "#bf{2017 #psi(2S)}");
  yl = getPos(h_y[0]->GetMinimum(), h_y[0]->GetMaximum(), 0.425, 1);
  ...
  lcyr.DrawLatex(-1.1, yl, "#bf{2017 #psi(2S)}");
  yl = getPos(h_yr[0]->GetMinimum(), h_yr[0]->GetMaximum(), 0.9, 0);
  ...
  lclt.DrawLatex(125, yl, "#bf{2017 #psi(2S)}");
  ...
  lcltf.DrawLatex(300, 1.5e6, "2017 Data");
  ...
  lcm.DrawLatex(3.425, 0.037, "#bf{2017 #psi(2S)}");
  ...
  lcmf.DrawLatex(3.15, 1e6, "2017 Data");
