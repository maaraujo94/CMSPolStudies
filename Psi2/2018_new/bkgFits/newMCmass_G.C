...
void newMCmass_G()
{
  ...
  for(int ip = 0; ip < nPtBins; ip++)
    h_m1d[ip] = new TH1D(Form("mH%.0f", ptBins[ip]), Form("2018 MC M(#mu#mu) (%.0f < p_{T} < %.0f GeV)",  ptBins[ip], ptBins[ip+1]), mbins, lowm, him);
  ...
  double par_v[] = {1., 0.7, 3.686, 1e-2, 3e-2, 1.1, 2.1, 0.1, 1e-1};
