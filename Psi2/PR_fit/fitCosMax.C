...
void fitCosMax()
{
  ...
  // read the coarse histos in |costh| to get binning
  ...
  double costhLim[nBinsY], ptLims[nBinsY+1], aux;
  ...
  // the histogram
  TH1F* costh = new TH1F("name", "cos#theta limits", nBinsY, ptLims);
  ...
  TF1 *fitf = new TF1("fitf", "[0]*log([1]+[2]*x)", ptLims[0], ptLims[nBinsY]);
