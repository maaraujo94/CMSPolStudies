...
void ltBkg2d_tnp2()
{
  ...
  double par_v[] =  {1.e6, 0.65,  3.5e-1,  5.5e-2};
  ...
  for(int i = 0; i < nmBins; i++) {
	...
	fitS->FixParameter(j*nmBins+i, par_v[j]);
      else if(j==1 && i > 3) fitS->SetParameter(j*nmBins+i, 0.3);
    }
  }
  ...
  for(int i_m = 0; i_m < nmBins; i_m++) {
    ...
    for(int j = 0; j < 4; j++) { // all free but last
      if(j!=3) {
    ...
    lc.DrawLatex(xp, yp, "#bf{2d fit with fixed t_{NP2}}");
    // draw the chi^2/ndf
    ...
    c->SaveAs(Form("plots/lifetime2d_tnp2/fit_%s.pdf", lbl[i_m].c_str()));
    c->Clear();
    ...
    c->SaveAs(Form("plots/lifetime2d_tnp2/pulls_%s.pdf", lbl[i_m].c_str()));
    c->Clear();
    ...
    c->SaveAs(Form("plots/lifetime2d_tnp2/devs_%s.pdf", lbl[i_m].c_str()));
    c->Clear();
  }
  
  // storing the free parameters
  TFile *fout = new TFile("files/ltfitres2d_tnp2.root", "recreate");
