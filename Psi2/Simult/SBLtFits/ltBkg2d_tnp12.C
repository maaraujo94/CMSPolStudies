...
double func_sum(double *xx, double *pp)
{
  ...
  /* double N_NP = pp[0]*d_m*exp(-m/pp[1]);
     double f1 = pp[nmBins+m_bin];*/
...
void ltBkg2d_tnp12()
{
  ...
  double par_v[] =  {1.e6, 0.5,  3.5e-1,  5.5e-2};
  double par2_v[] = {1.,   1.,   1.,      1.};
  ...
  for(int i_m = 0; i_m < nmBins; i_m++) {
    ...
    /*   pars[0][i_m] = 2.*m_err[i_m]*fitS->GetParameter(0)*exp(-m_val[i_m]/fitS->GetParameter(1)); // exp
	 pars[1][i_m] = fitS->GetParameter(nmBins+i_m); // free
	 pars[2][i_m] = fitS->GetParameter(2*nmBins); // constant
	 pars[3][i_m] = fitS->GetParameter(3*nmBins); // constant

	 epars[0][i_m] = pars[0][i_m]*sqrt(pow(fitS->GetParError(0)/fitS->GetParameter(0), 2) + pow(fitS->GetParError(1)*m_val[i_m]/(fitS->GetParameter(1)*fitS->GetParameter(1)), 2)); // exp
	 epars[1][i_m] = fitS->GetParError(nmBins+i_m); // free
	 epars[2][i_m] = fitS->GetParError(2*nmBins); // constant
	 epars[3][i_m] = fitS->GetParError(3*nmBins); // constant*/

    for(int j = 0; j < 4; j++) { // all free but tnp
      if(j<2) {
    ...
    lc.DrawLatex(xp, yp, "#bf{2d fit with fixed t_{NP1,2}}");
    // draw the chi^2/ndf
    ...
    c->SaveAs(Form("plots/lifetime2d_tnp12/fit_%s.pdf", lbl[i_m].c_str()));
    c->Clear();
    ...
    c->SaveAs(Form("plots/lifetime2d_tnp12/pulls_%s.pdf", lbl[i_m].c_str()));
    c->Clear();
    ...
    c->SaveAs(Form("plots/lifetime2d_tnp12/devs_%s.pdf", lbl[i_m].c_str()));
    c->Clear();
  }
  
  // storing the free parameters
  TFile *fout = new TFile("files/ltfitres2d_tnp12.root", "recreate");
