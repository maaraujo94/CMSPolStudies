...
void ltBkg2d_tnp12_N_f()
{
  ...
  for(int i = 0; i < nmBins; i++) {
    for(int j = 0; j < 4; j++) {
      ...
      // setting the linear and exponential parameters
      if(j<2 && i == 1) fitS->SetParameter(j*nmBins+i, par2_v[j]);
      else if(j<2 && i > 1) fitS->FixParameter(j*nmBins+i, par_v[j]);
  ...
  for(int i_m = 0; i_m < nmBins; i_m++) {
	...
	double devp1 = 2.*m_err[i_m]*(fitS->GetParameter(0)+sigp1/ln)*exp(-m_val[i_m]/fitS->GetParameter(1));
	double devp2 = 2.*m_err[i_m]*fitS->GetParameter(0)*exp(-m_val[i_m]/(fitS->GetParameter(1)+sigp2/ln));
    ...
    lc.DrawLatex(xp, yp, "#bf{2d fit with fixed t_{NP1,2}, f_{1} and N_{NP}}");
    // draw the chi^2/ndf
    ...
    c->SaveAs(Form("plots/lifetime2d_tnp12_N_f/fit_%s.pdf", lbl[i_m].c_str()));
    c->Clear();
    ...
    c->SaveAs(Form("plots/lifetime2d_tnp12_N_f/pulls_%s.pdf", lbl[i_m].c_str()));
    c->Clear();
    ...
    c->SaveAs(Form("plots/lifetime2d_tnp12_N_f/devs_%s.pdf", lbl[i_m].c_str()));
    c->Clear();
  }
  
  // storing the free parameters
  TFile *fout = new TFile("files/ltfitres2d_tnp12_N_f.root", "recreate");
