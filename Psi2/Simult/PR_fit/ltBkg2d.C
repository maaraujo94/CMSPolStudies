...
double func_sum(double *xx, double *pp)
{
  ...
  double f_bkg = pp[10*nPtBins+pt_bin]; // fixed in each bin
  double tnp1 = pp[11*nPtBins+pt_bin]; // fixed in each bin
  double tnp2 = pp[12*nPtBins+pt_bin]; // fixed in each bin
  ...
  fNP_bkg->SetParameters(1, f1, f2, mu, sig1, sig2, sig3, f_bkg, tnp1, tnp2);

  // scale N_bkg par
  fexp_bkg->SetParameters(N_bkg, f_bkg, tnp1, tnp2);
...
double nbkg_eval(double N_PR, double N_NP, double f1, double f2, double mu, double sig1, double sig2, double sig3, double tnp_SR, double N_bkg, double f_bkg, double tnp1, double tnp2)
{
  fNP_bkg->SetParameters(1, f1, f2, mu, sig1, sig2, sig3, f_bkg, tnp1, tnp2);
...
{
  double lt = xx[0];
  double N_PR = pp[0], N_NP = pp[1], f1 = pp[2], f2 = pp[3], mu = pp[4], sig1 = pp[5], sig2 = pp[6], sig3 = pp[7], tnp_SR = pp[8], N_bkg = pp[9], f_bkg = pp[10], tnp1 = pp[11], tnp2 = pp[12];

  fres->SetParameters(N_PR, f1, f2, mu, sig1, sig2, sig3);
  fNP_SR->SetParameters(N_NP, f1, f2, mu, sig1, sig2, sig3, tnp_SR);
  fNP_bkg->SetParameters(N_bkg, f1, f2, mu, sig1, sig2, sig3, f_bkg, tnp1, tnp2);
...
void ltBkg2d()
{
  ...
  TF1 *fexp2 = new TF1("fexp2", "[0]*pos_exp_p(x,[1]) + (1.-[0])*pos_exp_p(x,[2])", 5*lowt, 5*hit);
  ...
  TF2 *fitS = new TF2("fitS", func_sum, lowt, hit, ptBins[0], ptBins[nPtBins], 13*nPtBins, 2);
  string par_n[] = {"N_PR", "N_NP", "f1", "f2", "mu", "sigma1", "sig2/sig1", "sig3/sig1", "t_NPSR", "N_bkg", "f_bkg", "t_NP1", "t_NP2"};
  double par_v[] =  {1, 1, 0.55, 0.4, 0, 1e-2, 1.5, 3.5, 0.3, 1, 1, 1, 1};
  ...
  TF1 *f_1d = new TF1("f_1d", sum_1d, lowt, hit, 13);

  double pt_val[nPtBins], pt_err[nPtBins];
  double pars[13][nPtBins], epars[13][nPtBins];
  double rpars[13][nPtBins];
  ...
  for(int i_pt = 0; i_pt < nPtBins; i_pt++) {
    ...
    double N_bkg_conv = nbkg_eval(rpars[0][i_pt], rpars[1][i_pt], rpars[2][i_pt], rpars[3][i_pt], rpars[4][i_pt], rpars[5][i_pt], rpars[6][i_pt], rpars[7][i_pt], rpars[8][i_pt], rpars[9][i_pt], rpars[10][i_pt], rpars[11][i_pt], rpars[12][i_pt]);
    ...
    fNP_SR->Draw("lsame");
    // NP bkg
    fNP_bkg->SetParameters(N_bkg_conv, rpars[2][i_pt], rpars[3][i_pt], rpars[4][i_pt], rpars[5][i_pt], rpars[6][i_pt], rpars[7][i_pt], rpars[10][i_pt], rpars[11][i_pt], rpars[12][i_pt]);
    ...
    c->SaveAs(Form("plots/lifetime2d/fit/fit_%.0f.pdf", ptBins[i_pt]));
    c->Clear();
    ...
    for(int i_t = 0 ; i_t < tbins; i_t++) {
      ...
      if(datau > 0 && tv[i_t] < hit) pv[i_t] = (datav-fitv)/datau;
      else pv[i_t] = 0;
      if(fitv > 0 && tv[i_t] < hit) dv[i_t] = (datav-fitv)/fitv * 100;
      else dv[i_t] = 0;
    }
    ...
    c->SaveAs(Form("plots/lifetime2d/fit/pulls_%.0f.pdf", ptBins[i_pt]));
    c->Clear();
    ...
    fd->SetTitle(Form("Lifetime rel. difference (%.0f < p_{T} < %.0f GeV)", ptBins[i_pt], ptBins[i_pt+1]));
    ...
    c->SaveAs(Form("plots/lifetime2d/fit/devs_%.0f.pdf", ptBins[i_pt]));
    c->Clear();
  }
  
  // storing the free parameters
  TFile *fout = new TFile("files/ltfitres2d.root", "recreate");
  string parlab[] = {"N_PR", "N_NP", "f1", "f2", "mu", "sig1", "sigR21", "sigR31", "t_NPSR", "N_bkg", "f_bkg", "t_NP1", "t_NP2"};
