...
void plotCosFit()
{
  ...
  for(int i = 0; i < nBinsY; i++) {
    ...
    lcr1.DrawLatex(0.15, h_rat1d[1][i]->GetMaximum()*0.8, "(non-prompt #psi(2S))/MC");
  ...
  for(int i = 0; i < nBinsY; i++) {
    ...
    double mval = h_rat1d2[0][i]->GetBinContent(1)*1.6;
    h_rat1d2[0][i]->SetMaximum(h_rat1d2[0][i]->GetBinContent(1)*1.6);
    ...
    lcb1.DrawLatex(0.7, mval*0.9, "Run 2");
    lcb1.DrawLatex(0.7, mval*0.85, Form("%.0f-%.0f GeV", pMin, pMax));


    lcb1.SetTextColor(cols2[0]);
    lcb1.DrawLatex(0.15, h_rat1d2[0][i]->GetMaximum()*0.775, "PRS/MC");
