...
void plotLth_NP()
{
  ...
  // [0]: dR > 0.17 -> 20 to pt_c GeV
  // [1]: dR > 0.15 -> pt_c to 100 GeV
  double* binsX = graph_lth[0]->GetX();
  int cut_val;
  for(int i = 1; i < nBinspT; i++) {
    if(binsX[i] > pt_c && binsX[i-1] < pt_c) cut_val = i;
  }
  ...
  fl2->SetTitle("Run 2 #delta#lambda_{#theta} (NP #psi(2S))");
  ...
  zero->Draw();
  TLine *trans2D = new TLine(pt_c, -d_lim, pt_c, d_lim);
  ...
  fl2_c->SetTitle("Run 2 #delta#lambda_{#theta} (NP #psi(2S))");
  ...
  fl3->SetTitle("Run 2 #lambda_{#theta} (NP #psi(2S))");
  ...
  zero->Draw();
  TLine *trans2A = new TLine(pt_c, -a_lim, pt_c, a_lim);
  ...
  fl3_c->SetTitle("Run 2 #lambda_{#theta} (NP #psi(2S))");
