...
void getfL()
{
  ...
  TFile *inFMass = new TFile("../bkgFits/files/ctFitRes_NP.root");
  TGraphErrors *m_b = (TGraphErrors*)inFMass->Get("graph_b");
  inFMass->Close();

  // get the binning
  int nBinsY = m_b->GetN();
  const double *yBins = m_b->GetX();
  ...
  for(int i = 0; i < nBinsY; i++) {
    ...
    fMass->SetParameter(0, m_b->GetY()[i]);
    mMass->SetParameter(0, m_b->GetY()[i]);
