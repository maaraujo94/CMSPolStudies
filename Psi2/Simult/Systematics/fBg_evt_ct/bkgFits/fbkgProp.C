...
void fbkgProp()
{
  ...
  h_LSB = (TH1D*)fin->Get("lsbH");
  h_LSB->SetDirectory(0);
  h_RSB = (TH1D*)fin->Get("rsbH");
  h_RSB->SetDirectory(0);
  h_tot = (TH1D*)fin->Get("totH");
  ...
  TH1D *h_SR = new TH1D("srH", "Run 2 Data (SR)", nPtBins, ptBins);
  ...
  for(int i = 0; i < nPtBins; i++) {
    ...
    fit->SetParameter(0, "A", dataInt[0], dataInt[0]/100., 0, 0);
    fit->SetParameter(1, "b", 3, 0.1, 0, 0);
  ...
  TFile *fout_fr = new TFile("files/ctFitRes.root", "recreate");
  g_A->SetName("graph_A");
  g_A->Write();
  g_b->SetName("graph_b");
  g_b->Write();
  ...
  TFile *finBg = new TFile("../../../bkgFits/files/bkgFrac.root");
  ...
  TFile *fout_f = new TFile("files/bkgFrac.root", "recreate");
  f_SB->SetTitle("Run 2 f_{Bg} from yield ratio");
