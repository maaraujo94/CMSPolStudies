void calcSys()
{
  ...
  for(int i = 0; i < nBinspT; i++) {
    ...
    cout << pTBins[i] << "-" << pTBins[i+1] << ": " << diff78 << ", " << diff78*(pTBins[i+1]-pTBins[i])/(pTBins[nBinspT]-pTBins[0]) << " (2017) / ";
    
    diff78 = val8-graph_lth[0]->GetY()[i];
    v_int[1] += diff78*(pTBins[i+1]-pTBins[i])/(pTBins[nBinspT]-pTBins[0]);
    
    cout  << diff78 << ", " << diff78*(pTBins[i+1]-pTBins[i])/(pTBins[nBinspT]-pTBins[0]) << " (2018)" << endl;    
  }
  ...
  THStack *hsigN = new THStack("hsigN", "Stacked #sigma^{2}");
  ...
  ftex << "$\\pt$ (GeV) & $\\sigma^{\\text{comb}}$ & $\\sigma^{\\text{eff}}$ & $\\sigma^\\rho$ & $\\sigma_{\\text{sys}}$ & $\\sigma_{\\text{stat}}$ \\\\\n";
  ftex << "\\hline\n";

  int p_norm = 4;
  double val_d, val_eff, val_rho;
  int rho_lims[4] = {0, 3, 5, 6};
  for(int i = 0; i < nBinspT; i++) {
    // pT bin
    ...
    double valP = sqrt(pow(val_d,2)+pow(val_eff,2)+pow(val_rho,2));
  ...
  fl1->SetTitle("Run 2 #lambda_{#theta} (prompt #psi(2S))");
