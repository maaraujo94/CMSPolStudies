...
void fbkgProp_NP()
{
  ...
  TH1D *h_fbkg = new TH1D("h_fbkg", "Run 2 f_{NPBg}", nPtBins, ptBins);
  ...
  for(int i_pt = 0; i_pt < nPtBins; i_pt++) {
    ...
    ef_Bg[i_pt] = parErr(n_par, intfbkg, 0, par_vec, epar_vec, cov_ptr)/evt_all * 100.;
  ...
  TH1F *fr1 = c->DrawFrame(ptBins[0]-5, 0, ptBins[nPtBins]+5, 100);
  ...
  TH2D *h_fbkg2d = new TH2D("h_fbkg2d", "Run 2 f_{bkg}", nBinsX, minX, maxX, nPtBins, ptBins);
  for(int i_pt = 0; i_pt < nPtBins; i_pt++) {
