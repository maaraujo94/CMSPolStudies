...
void newDatamass_2()
{
  ...
  double par_v[] =  {1.,   0.55, 3.7,  1e-4,   1e-4,   n_v, alpha_v, 0.1,   1.};
  ...
  f_1d->SetParNames("NS", "f", "mu", "sigma1", "sigma2", "n", "alpha", "m_bkg", "b_bkg");
  f_1d->SetNpx(500);
  ...
  for(int i_pt = 0; i_pt < nPtBins; i_pt++) {
    ...
    c->SaveAs(Form("plots/mass/fit_pt%d_log.pdf", i_pt));
    c->Clear();
