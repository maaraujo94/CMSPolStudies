...
double mmod_func(double *x, double *par)
{
  ...
  double func = f * cb_exp(m, NS, sig1, mu, n, alpha) + (1.-f) * cb_exp(m, NS, sig2, mu, n, alpha) + bkg_exp(m, m_bkg, b_bkg);
  return func;
}
...
void newNPmass()
{
  ...
  for(int i = 0; i < nPtBins; i++) {
    ...
    h_d1d[i]->SetTitle(Form("Run 2 NP M(#mu#mu) (%.0f < p_{T} < %.0f GeV)", ptBins[i], ptBins[i+1]));
  }
  ...
  string par_n[] =  {"NS", "f",  "mu",  "sig1", "sig2", "n", "alpha", "m_bkg", "b_bkg"};
  double par_v[] =  {1., 0.55, 3.7, 1e-4, 1e-4, n_v, alpha_v, 0.25, 1};
  ...
  for(int i = 0; i < nPtBins; i++) {
    ...
    f_cb->SetParameter(8*nPtBins+i, h_d1d[i]->GetBinContent(2)*10);
  ...
  for(int i_pt = 0; i_pt < nPtBins; i_pt++) {
    ...
    c->SaveAs(Form("plots/massNP/fit_pt%d.pdf", i_pt));
    c->Clear();
    ...
    c->SaveAs(Form("plots/massNP/fit_pt%d_log.pdf", i_pt));
    c->Clear();
    ...
    c->SaveAs(Form("plots/massNP/pulls_pt%d.pdf", i_pt));
    c->Clear();
    ...
    c->SaveAs(Form("plots/massNP/devs_pt%d.pdf", i_pt));
    c->Clear();
  }
  
  // storing the free parameters
  TFile *foutF = new TFile("files/mfit_NP.root", "recreate");
  string parlab[] = {"NS", "f", "mu", "sig1", "sig2", "n", "alpha", "m_bkg", "b_bkg"};
