...
void fnpProp()
{
  ...
  // plotting in pT - compare with baseline
  TFile *finB = new TFile("../../PR_fit/files/NPFrac.root");
  TH1D *h_fnp_B = (TH1D*)finB->Get("fnppsi_unc");
  h_fnp_B->SetDirectory(0);
  finB->Close();
  ...
  h_fnp_B->SetStats(0);
  h_fnp_B->Scale(100);
  h_fnp_B->SetMarkerStyle(20);
  h_fnp_B->SetMarkerSize(.5);
  h_fnp_B->SetMarkerColor(kBlack);
  h_fnp_B->SetLineColor(kBlack);
  h_fnp_B->SetLineStyle(kDashed);
  h_fnp_B->Draw("e1 same");
