...
// n, fG fixed (fG from MC)
// f, mu constant in pT
// sigma_1,2,G linear in pT (same slope)
// alpha, N and lambda free
...
void newDatamass_0()
{
  ...
  double par_v[] =  {1.,   0.55, 3.65,  1e-4,   1e-4,   n_v, alpha_v, 1.,   2.};
  double par2_v[] = {1.,   1.,  1.,   2e-2,   3.5e-2,   1.,  1.,      1.,   1.};
  ...
  for(int i_pt = 0; i_pt < nPtBins; i_pt++) {
	...
	epars[j][i_pt] = f_cb->GetParError(j*nPtBins+i_pt);
      ...
      else if ( j == 1 || j == 2 || j == 5 ) { // constant parameters f, mu, n
    ...
    c->SaveAs(Form("plots/mass/fit_0/fit_pt%d.pdf", i_pt));
    c->Clear();
    ...
    c->SaveAs(Form("plots/mass/fit_0/fit_pt%d_log.pdf", i_pt));
    c->Clear();
    ...
    c->SaveAs(Form("plots/mass/fit_0/pulls_pt%d.pdf", i_pt));
    c->Clear();
    ...
    c->SaveAs(Form("plots/mass/fit_0/devs_pt%d.pdf", i_pt));
    c->Clear();
  }
  
  // storing the free parameters
  TFile *fout = new TFile("files/mfit_0.root", "recreate");
