...
void ltPerPt_N(double binLo, double binHi, int nBin)
{
  ...
  TH2D *h_d2d = new TH2D(Form("ltH_%d", nBin), "Run 2 data c#tau (sidebands)", tbins, lowt, hit, nmBins+1, mBins);
  ...
  double par_v[] =  {1.e6, f1_1,  tnp1,  tnp2};
  double par2_v[] = {1.,   f1_2,   1.,      1.};
  ...
  for(int i = 0; i < nmBins; i++) {
    for(int j = 0; j < 4; j++) {
      ...
      // fixing the linear parameter
      else if(j==1 && i == 1) fitS->FixParameter(j*nmBins+i, par2_v[j]);
  ...
  for(int i_m = 0; i_m < nmBins; i_m++) {
	...
	epars[j][i_m] = sqrt(pow(fitS->GetParError(j*nmBins+1) * m_val[i_m], 2) + pow(fitS->GetParError(j*nmBins), 2));
      }
      else{
    ...
    lc.DrawLatex(xp, yp, Form("#bf{%.0f < #it{p}_{T} < %.0f GeV}", binLo, binHi));
