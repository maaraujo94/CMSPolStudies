...
void fitBkg()
{
  ...
  for(int i_fit = 0; i_fit < n_fit; i_fit++)
	...
	pullR[i] = new TH1D(Form("pullR_%d_%d", i_fit, i), Form("2017 fit pulls (%.0f < p_{T} < %.0f GeV)", yBins[i], yBins[i+1]), nBinsX, minX, maxX);
	devL[i] = new TH1D(Form("devL_%d_%d", i_fit, i), Form("2017 fit deviation (%.0f < p_{T} < %.0f GeV)", yBins[i], yBins[i+1]), nBinsX, minX, maxX);
	devR[i] = new TH1D(Form("devR_%d_%d", i_fit, i), Form("2017 fit deviation (%.0f < p_{T} < %.0f GeV)", yBins[i], yBins[i+1]), nBinsX, minX, maxX);
