...
void binnedSub()
{
  ...
  for(int i_pt = 0; i_pt < nBinsY; i_pt++) {
    ...
    h_PM[i_pt] = new TH1D(Form("h_PM_%d", i_pt), Form("2018 #psi(2S) Mass (%.0f < p_{T} < %.0f GeV)", pt_min, pt_max), nbins[i_pt], minH[i_pt], maxH[i_pt]);
  }
  TGraph **g_pull = new TGraph*[nBinsY];
  TGraph **g_dev = new TGraph*[nBinsY];
  
  // get data tree, branches, etc
  TFile *infile = new TFile("../Store_data_codes/data18_cos.root");
  ...
  for(int i_pt = 0; i_pt < nBinsY; i_pt++)
	...
	if((m_l >= min && m_u <= mlc) || (m_l >= muc && m_u <= max)) {
      ...
      fc_p->SetTitle(Form("2018 Bkg fit pulls (%.0f < p_{T} < %.0f GeV)", pt_min, pt_max));
      ...
      fc_d->SetTitle(Form("2018 Bkg fit deviation (%.0f < p_{T} < %.0f GeV)", pt_min, pt_max));
  ...
  fc->SetTitle("2018 Fraction of bkg events in peak region");
  ...
  fcAN->SetTitle("2018 A/pT bin width (p_{T})");
  ...
  fcchi->SetTitle("2018 P(#chi^{2}, ndf)");
