...
void bkgCosth()
{
  ...
  for(int i = 0; i < 2; i++) {
    dataHist[i] = new TH2D(Form("dataH%d", i), Form("Run 2 Data (%s)", lbl[i].c_str()), 40, -1., 1., nPtBins, ptBins);
    dataHist_ab[i] = new TH2D(Form("dataH%d_ab", i), Form("Run 2 Data (%s)", lbl[i].c_str()), 20, 0, 1., nPtBins, ptBins);
  }
  
  TH2D *mcHist = new TH2D("mcH", "Run 2 MC", 40, -1., 1., nPtBins, ptBins);
  ...
  for(int i = 0; i < dEvt; i++)
    {
      treeD->GetEntry(i);
      if(data_pt > ptBins[0] && data_pt < ptBins[nPtBins] && abs(data_y) < 1.2 && data_lt > 0.01 && data_lt < 0.05 && (abs(mPEta) > eta_lim || mPPt > pt_cut) && (abs(mMEta) > eta_lim || mMPt > pt_cut)) {
	// LSB
  ...
  for(int i = 0; i < m1Evt; i++)
    {
      treeM1->GetEntry(i);
      if(mc_pt > ptBins[0] && mc_pt < 46 && abs(mc_lt) < 0.005 && mc_m < m_max[1] && mc_m > m_min[1] && abs(mc_y) < 1.2 && (abs(mPEta) > eta_lim || mPPt > pt_cut) && (abs(mMEta) > eta_lim || mMPt > pt_cut)) {
  ...
  for(int i = 0; i < m3Evt; i++)
    {
      treeM3->GetEntry(i);
      if(mc_pt > 46 && mc_pt < ptBins[nPtBins] && abs(mc_lt) < 0.005 && mc_m < m_max[1] && mc_m > m_min[1] && abs(mc_y) < 1.2 && (abs(mPEta) > eta_lim || mPPt > pt_cut) && (abs(mMEta) > eta_lim || mMPt > pt_cut)) {
	mcHist->Fill(cos(mc_th), mc_pt);
	mcHist_ab->Fill(abs(cos(mc_th)), mc_pt);
      }
    }
  ...
  for(int i = 0; i < 2; i++) {
    ratioHist_ab[i] = new TH2D(Form("ratioH%d_ab", i), Form("Run 2 Data/MC (%s)", lbl[i].c_str()), 20, 0, 1., nPtBins, ptBins);
    
    ratioHist_ab[i] = (TH2D*)dataHist_ab[i]->Clone(Form("ratioH%d_ab", i));
    ratioHist_ab[i]->Sumw2();
    ratioHist_ab[i]->Divide(mcHist_ab);
    ratioHist_ab[i]->SetTitle(Form("Run 2 Data/MC (%s)", lbl[i].c_str()));
