...
void histoSave()
{
  ...
  TH2D *dataHist = new TH2D("dataH", "Run 2 Data (PR)", 40, -1., 1., nPtBins, ptBins);
  TH2D *NPHist = new TH2D("NPH", "Run 2 Data (NP)", 40, -1., 1., nPtBins, ptBins);
  ...
  TH2D *NPHist_ab = new TH2D("NPH_ab", "Run 2 Data (NP)", 20, 0, 1., nPtBins, ptBins);
  ...
  for(int i = 0; i < dEvt; i++)
    {
      treeD->GetEntry(i);
      if(data_pt > ptBins[0] && data_pt < ptBins[nPtBins] && data_m > m_min && data_m < m_max && abs(data_y) < 1.2 && (abs(mPEta) > eta_lim || mPPt > pt_cut) && (abs(mMEta) > eta_lim || mMPt > pt_cut)) {
  ...
  for(int i = 0; i < m1Evt; i++)
    {
      treeM1->GetEntry(i);
      if(mc_pt > ptBins[0] && mc_pt < 46 && abs(mc_lt) < 0.005 && abs(mc_y) < 1.2 && mc_m > m_min && mc_m < m_max && (abs(mPEta) > eta_lim || mPPt > pt_cut) && (abs(mMEta) > eta_lim || mMPt > pt_cut)) {
  ...
  for(int i = 0; i < m3Evt; i++)
    {
      treeM3->GetEntry(i);
      if(mc_pt > 46 && mc_pt < ptBins[nPtBins] && abs(mc_lt) < 0.005 && abs(mc_y) < 1.2 && mc_m > m_min && mc_m < m_max && (abs(mPEta) > eta_lim || mPPt > pt_cut) && (abs(mMEta) > eta_lim || mMPt > pt_cut)) {
  ...
  TH2D *ratNPHist_ab = new TH2D("ratNPH_ab", "Run 2 NP/MC", 20, 0, 1., nPtBins, ptBins);
  ratNPHist_ab = (TH2D*)NPHist_ab->Clone("ratNPH_ab");
  ratNPHist_ab->Sumw2();
  ratNPHist_ab->Divide(mcHist_ab);
  ratNPHist_ab->SetTitle("Run 2 NP/MC");
