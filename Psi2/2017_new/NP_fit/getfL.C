...
void getfL()
{
  ...
  TF1 *fMass = new TF1("fMass", "exp(-x/[0])", m_min[0], m_max[2]);
  // define same function as above but *m
  TF1 *mMass = new TF1("mMass", "exp(-x/[0])*x", m_min[0], m_max[2]);
