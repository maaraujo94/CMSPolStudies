...
void bkgCosth()
{
  ...
  TFile *fin2 = new TFile("/home/mariana/Documents/2020_PhD_work/CERN/CMSPolStudies/Psi2/Store_data_codes/MC17_cos.root");
  TTree *treeM1 = (TTree*)fin2->Get("MC_cos");
  TFile *fin4 = new TFile("/home/mariana/Documents/2020_PhD_work/CERN/CMSPolStudies/Psi2/Store_data_codes/MCvh17_cos.root");
  ...
  for(int i = 0; i < 2; i++) {
    dataHist[i] = new TH2D(Form("dataH%d", i), Form("2017 Data (%s)", lbl[i].c_str()), 40, -1., 1., nPtBins, ptBins);
    dataHist_ab[i] = new TH2D(Form("dataH%d_ab", i), Form("2017 Data (%s)", lbl[i].c_str()), 20, 0, 1., nPtBins, ptBins);
  }
  ...
  for(int i = 0; i < 2; i++) {
    ratioHist_ab[i] = new TH2D(Form("ratioH%d_ab", i), Form("2017 Data/MC (%s)", lbl[i].c_str()), 20, 0, 1., nPtBins, ptBins);
    
    ratioHist_ab[i] = (TH2D*)dataHist_ab[i]->Clone(Form("ratioH%d_ab", i));
    ratioHist_ab[i]->Sumw2();
    ratioHist_ab[i]->Divide(mcHist_ab);
    ratioHist_ab[i]->SetTitle(Form("2017 Data/MC (%s)", lbl[i].c_str()));
