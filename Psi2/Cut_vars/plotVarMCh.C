// code to plot the cut variables for the MC Jpsi sample (high pt)
/* variables to plot
- single muon pT, eta
...
void plotVarMCh()
{
  TFile *infile = new TFile("../Store_data_codes/Psi2_MC_hpt_cuts.root");
  ...
  h_JMass->SetTitle("Inclusive MC (high p_{T}) #psi(2S) mass");
  h_JMass->Draw("hist");
  c->SaveAs("plots/MC_hpt_psi2_mass.pdf");
  c->Clear();
  ...
  h_JPt->SetTitle("Inclusive MC (high p_{T}) #psi(2S) p_{T}");
  h_JPt->Draw("hist");
  TLine *JPt_1 = new TLine(46, 2e1, 46, 5e6);
  ...
  JPt_2->Draw("lsame");
  c->SaveAs("plots/MC_hpt_psi2_pt.pdf");
  c->Clear();
  ...
  h_Jy->SetTitle("Inclusive MC (high p_{T}) #psi(2S) y");
  h_Jy->Draw("hist");
  ...
  JRap_2->Draw("lsame");
  c->SaveAs("plots/MC_hpt_psi2_rap.pdf");
  c->Clear();

  c->SetLogy();
  h_Jlts->GetYaxis()->SetRangeUser(1e4, 1e6);
  h_Jlts->GetXaxis()->SetTitle("|c#tau|/#sigma_{c#tau}");
  h_Jlts->SetTitle("Inclusive MC (high p_{T}) #psi(2S) lts");
  h_Jlts->Draw("hist");
  ...
  Jlts_2->Draw("lsame");
  c->SaveAs("plots/MC_hpt_psi2_lts.pdf");
